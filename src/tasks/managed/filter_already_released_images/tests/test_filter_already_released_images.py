...
from release_service_utils.tasks.managed import filter_already_released_images
from filter_already_released_images import (
    filter_already_released_images as _filter_already_released_images,
)

import pytest

TASK = "release_service_utils.tasks.managed.filter_already_released_images"
...
class TestCheckTag:
    ...
    def test_matching_digest_returns_true(self) -> None:
        ...
        with patch(
            f"{TASK}.filter_already_released_images.oras_resolve",
            return_value="sha256:abc",
        ):
            assert filter_already_released_images.filter_already_released_images._check_tag(
    ...
    def test_mismatched_digest_returns_false(self) -> None:
        ...
        with patch(
            ...
            assert (
                not filter_already_released_images.filter_already_released_images._check_tag(
                    "reg.io/repo", "v1", "sha256:abc"
                )
            )
    ...
    def test_passes_repo_url_as_auth_ref(self) -> None:
        ...
        with patch(
            ...
            filter_already_released_images.filter_already_released_images._check_tag(
                "reg.io/repo", "v1", "sha256:abc"
            )
...
class TestIsComponentReleased:
    ...
    def test_all_tags_match_returns_true(self) -> None:
        ...
        with patch(
            f"{TASK}.filter_already_released_images._check_tag",
            return_value=True,
        ):
...
class TestPartitionComponents:
    ...
    def test_all_released(self) -> None:
        ...
        with (
            patch(
                "filter_already_released_images.filter_already_released_images.oras_resolve",
                return_value="sha256:a1",
            ),
            patch(
                f"{TASK}.filter_already_released_images" ".is_component_released",
                return_value=True,
            ),
        ):
            kept, filtered = _filter_already_released_images._partition_components(components)
        assert filtered == 1
        assert kept == []
    ...
    def test_none_released(self) -> None:
        ...
        with (
            ...
            patch(
                "filter_already_released_images.filter_already_released_images"
    ...
    def test_empty_components(self) -> None:
        ...
        kept, filtered = _filter_already_released_images._partition_components([])
        assert filtered == 0
        assert kept == []
...
class TestFilterSnapshot:
    ...
    def test_overwrites_snapshot_in_place(self, tmp_path: Path) -> None:
        ...
        with patch(
            f"{TASK}.filter_already_released_images" "._partition_components",
...
class TestRun:
    ...
    def test_success(self, tmp_path: Path) -> None:
        ...
        with patch(
            f"{TASK}.filter_already_released_images.filter_snapshot",
            return_value=(1, 0),
        ):
...
class TestParseArgs:
    ...
    def test_snapshot_path_required(self) -> None:
        ...
        with pytest.raises(SystemExit):
            filter_already_released_images.filter_already_released_images._parse_args([])

    def test_parses_snapshot_path(self) -> None:
        """--snapshot-path is correctly parsed."""
        args = filter_already_released_images.filter_already_released_images._parse_args(
