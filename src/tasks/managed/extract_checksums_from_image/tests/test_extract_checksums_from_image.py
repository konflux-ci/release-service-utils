...
def test_extract_checksums_temp_dir_cleaned_on_failure(
    ...
    original_mkdtemp = ecfi.extract_checksums_from_image.tempfile.mkdtemp
    ...
    with mock.patch.object(
        ecfi.extract_checksums_from_image.tempfile, "mkdtemp", _tracking_mkdtemp
    ):
        with pytest.raises(subprocess.CalledProcessError):
            ecfi.extract_checksums(
                snapshot_path,
...
def test_main_writes_result(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    with mock.patch.object(
        ecfi.extract_checksums_from_image, "extract_checksums", return_value="uid/binaries"
    ):
        rc = ecfi.main()
...
def test_main_extract_error_raises(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    with mock.patch.object(
        ecfi.extract_checksums_from_image, "extract_checksums", side_effect=ValueError("boom")
    ):
