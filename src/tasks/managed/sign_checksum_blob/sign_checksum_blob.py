...
class SigningConfig:
    """Store signing values resolved from the ConfigMap and release data."""
...
def resolve_signing_config(data_file: Path) -> SigningConfig:
    """Return signing values from the ConfigMap and direct signing options.

    Reads ``SIG_KEY_NAME`` from the signing ConfigMap selected by
    ``data.sign.configMapName``. Kerberos settings are supplied by
    ``data.signOptions``.
    """
    ...
    keyname = configmap["data"]["SIG_KEY_NAME"]
    sign_options = data.get("signOptions", {})
    keytab = sign_options.get("signKeytab", {})
    return SigningConfig(
        keyname=keyname,
        kerberos_keytab_secret=keytab.get("secret", ""),
        kerberos_keytab=keytab.get("name", ""),
        kerberos_principal=keytab.get("principal", ""),
    )
