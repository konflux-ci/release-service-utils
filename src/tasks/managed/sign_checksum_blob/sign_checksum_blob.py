...
When a signature already exists in the restored workspace, the catalog skips
signing if it is parseable; otherwise it removes the signature and re-signs.
Each TaskRun retry restores its own workspace, so this does not reuse signing
results from an earlier attempt.
"""
...
from release_service_utils.helpers.direct_sign_generic import direct_sign_generic
...
class SigningConfig:
    """Store signing values resolved from the ConfigMap."""
...
def resolve_signing_config(data_file: Path) -> SigningConfig:
    """Return signing values from the selected ConfigMap.

    The ConfigMap is selected by ``data.sign.configMapName`` or the default
    signing ConfigMap name. It must contain the signing key and Kerberos
    settings consumed by the generic-signing pipeline.
    """
    ...
    return SigningConfig(
        keyname=configmap_data["SIG_KEY_NAME"],
        kerberos_keytab_secret=configmap_data["KERBEROS_KEYTAB_SECRET"],
        kerberos_keytab=configmap_data["KERBEROS_KEYTAB"],
        kerberos_principal=configmap_data["KERBEROS_PRINCIPAL"],
    )
...
def sign(
    ...
    request = direct_sign_generic.SigningRequest(
        ...
        oci_artifact_expires_after=oci_artifact_expires_after,
        trusted_artifacts_debug=trusted_artifacts_debug,
        data_dir=data_dir,
        ta_task_git_url=ta_task_git_url,
        ta_task_git_revision=ta_task_git_revision,
        ca_trust_config_map_name=ca_trust_config_map_name,
        ca_trust_config_map_key=ca_trust_config_map_key,
        trusted_artifacts_dockerconfig_secret=trusted_artifacts_dockerconfig_secret,
        verbose=verbose,
        task_id=task_id,
...
def main() -> int:
    ...
    if args.command == "prepare":
        ...
        signature_valid_result, checksum_file_result = tekton.result_paths_from_env(
            "SIGNATURE_VALID_RESULT", "CHECKSUM_FILE_RESULT"
        )
        signature_valid_result.write_text(str(signature_valid).lower(), encoding="utf-8")
        checksum_file_result.write_text(checksum_name, encoding="utf-8")
        return 0
    ...
    if args.command == "sign":
        ...
        sign(
            ...
            oci_artifact_expires_after=os.environ.get(
                "OCI_ARTIFACT_EXPIRES_AFTER", ""
            ).strip(),
            trusted_artifacts_debug=os.environ.get("TRUSTED_ARTIFACTS_DEBUG", "").strip(),
            data_dir=str(data_dir),
            ca_trust_config_map_name=os.environ.get("CA_TRUST_CONFIG_MAP_NAME", "").strip(),
            ca_trust_config_map_key=os.environ.get("CA_TRUST_CONFIG_MAP_KEY", "").strip(),
            trusted_artifacts_dockerconfig_secret=os.environ.get(
                "TRUSTED_ARTIFACTS_DOCKERCONFIG_SECRET", ""
            ).strip(),
            verbose=os.environ.get("VERBOSE", "").strip().lower() == "true",
            request_timeout=int(tekton.require_env("REQUEST_TIMEOUT")),
            service_account=tekton.require_env("SIGN_PIPELINE_SERVICE_ACCOUNT"),
        )
        return 0
