...
def test_resolve_signing_config_reads_configmap_values(tmp_path) -> None:
    """resolve_signing_config reads the signing key and Kerberos settings."""
    data = {}
    ...
    assert config.kerberos_keytab == "keytab"
    assert config.kerberos_principal == "signer@EXAMPLE.COM"
    configmap.assert_called_once_with("signing-config-map")


def test_resolve_signing_config_requires_configmap_settings(tmp_path) -> None:
    """Missing ConfigMap Kerberos settings raise a clear key error."""
    data = {}
    with patch(f"{MODULE}.file.load_json_dict", return_value=data):
        with patch(
            f"{MODULE}.kubectl.get_configmap",
            ...
            with pytest.raises(KeyError, match="KERBEROS_KEYTAB_SECRET"):
                resolve_signing_config(tmp_path / "data.json")


def test_resolve_signing_config_uses_custom_config_map(tmp_path) -> None:
    """A custom signing ConfigMap is honored."""
    ...
    configmap.assert_called_once_with("custom-cm")
...
def test_main_prepare_writes_result_files(tmp_path, monkeypatch) -> None:
    """Prepare writes its outputs to the configured Tekton result paths."""
    signature_result = tmp_path / "signature-valid"
    checksum_result = tmp_path / "checksum-file"
    monkeypatch.setenv("SIGNATURE_VALID_RESULT", str(signature_result))
    monkeypatch.setenv("CHECKSUM_FILE_RESULT", str(checksum_result))

    env_values = {
        "DATA_DIR": str(tmp_path),
        "BINARIES_PATH": "binaries",
        "ISOLATED_INPUT_DIR": "isolated",
    }
    with (
        patch(f"{MODULE}.setup_argparser") as setup_argparser,
        patch(f"{MODULE}.tekton.require_env", side_effect=env_values.__getitem__),
        patch(f"{MODULE}.prepare", return_value=(False, "SHA256SUMS")),
    ):
        setup_argparser.return_value.parse_args.return_value.command = "prepare"
        assert main() == 0

    assert signature_result.read_text() == "false"
    assert checksum_result.read_text() == "SHA256SUMS"
...
def _sign_kwargs(data_file: Path, output_file: Path) -> dict[str, str | Path | int]:
    return dict(
        data_file=data_file,
...
def test_sign_submits_and_records_output(tmp_path) -> None:
    ...
    assert request.kerberos_keytab == "keytab"
    assert request.kerberos_principal == "signer@EXAMPLE.COM"
    assert request.service_account == "signing-pipeline-sa"
    assert request.oci_artifact_expires_after == "1d"
    assert request.trusted_artifacts_debug == "true"
    assert request.data_dir == str(tmp_path)
    assert request.ca_trust_config_map_name == "trusted-ca"
    assert request.ca_trust_config_map_key == "ca-bundle.crt"
    assert request.trusted_artifacts_dockerconfig_secret == "dockerconfig"
...
def test_finalize_dearmors_with_real_gpg(tmp_path) -> None:
    ...
    fixture = Path(__file__).parent / "test_data" / "armored-signature.asc"
