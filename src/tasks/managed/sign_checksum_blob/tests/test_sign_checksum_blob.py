...
def test_resolve_signing_config_reads_direct_signing_options(tmp_path) -> None:
    """resolve_signing_config reads the ConfigMap key and Kerberos options."""
    data = {
        "signOptions": {
            "signKeytab": {
                "secret": "stage-keytab",
                "name": "/etc/kerberos/keytab",
    ...
    with patch(f"{MODULE}.file.load_json_dict", return_value=data):
        with patch(
            ...
            return_value={"data": {"SIG_KEY_NAME": "prodkey"}},
        ) as configmap:
...
def test_resolve_signing_config_uses_defaults_for_optional_values(tmp_path) -> None:
    """Optional Kerberos values default without signOptions."""
    data = {"signOptions": {}}
    with patch(f"{MODULE}.file.load_json_dict", return_value=data):
        with patch(
            f"{MODULE}.kubectl.get_configmap",
    ...
    assert config.kerberos_keytab == ""
    assert config.kerberos_principal == ""
