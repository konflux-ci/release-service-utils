...
from release_service_utils.tasks.managed import publish_to_mrrc_push_merged
...
def test_push_merged_happy_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        publish_to_mrrc_push_merged.publish_to_mrrc_push_merged.subprocess,
        "check_output",
        fake_check_output,
    )
    monkeypatch.setattr(
        publish_to_mrrc_push_merged.publish_to_mrrc_push_merged.oras_utils,
        "oras_resolve",
        lambda ref: "sha256:abcdef1234567890",
    )
    monkeypatch.setattr(
        publish_to_mrrc_push_merged.publish_to_mrrc_push_merged,
        "generate_tag",
