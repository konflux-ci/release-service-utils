...
from release_service_utils.tasks.managed.create_github_release import (
    create_github_release as cgr,
)
...
class TestCheckReleaseExists:
    ...
    def test_release_exists_returns_url(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Return release URL when jq filter finds a matching release."""
        release = {"tag_name": "v1.2.3"}

        def mock_gh(cmd, *, gh_token, check=True, cwd=None):
            assert "repos/foo/bar/releases" in cmd[2]
            assert "--jq" in cmd
            return _make_completed_process(stdout=json.dumps(release))

        monkeypatch.setattr(github, "run_gh_command", mock_gh)
        result = cgr.check_release_exists(
            "foo/bar", "1.2.3", "token", "https://github.com/foo/bar"
        )
        assert result == "https://github.com/foo/bar/releases/tag/v1.2.3"

    def test_release_not_found_returns_none(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Return None when jq filter returns empty output."""
    ...
    def test_whitespace_only_output_returns_none(
        ...
        """Return None when jq filter returns only whitespace."""
...
class TestMain:
    ...
    def test_successful_run(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        ...
        monkeypatch.setenv("RESULT_RELEASE_URL", str(result_url))
    ...
    def test_missing_result_env_raises(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Missing RESULT_RELEASE_URL causes SystemExit."""
        monkeypatch.delenv("RESULT_RELEASE_URL", raising=False)
    ...
    def test_missing_repository_raises(
        ...
        monkeypatch.setenv("RESULT_RELEASE_URL", str(tmp_path / "r"))
