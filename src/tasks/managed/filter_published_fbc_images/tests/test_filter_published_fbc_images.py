...
def skip_openshift_version_label() -> Iterator[None]:
    """Treat the FBC openshift.version label as absent unless a test overrides it."""
    with patch(f"{TASK}.read_openshift_version_label", return_value=None):
        yield
...
def test_extract_component_ocp_version_uses_label() -> None:
    """Prefer a valid openshift.version label over the base-image annotation."""
    with (
        patch(f"{TASK}.read_openshift_version_label", return_value=["v4.21"]),
        patch(f"{TASK}.resolve_ocp_version", return_value="4.15") as mock_annotation,
    ):
        assert fbc.extract_component_ocp_version(_component("comp", "sha256:abc")) == "v4.21"
    mock_annotation.assert_not_called()


def test_extract_component_ocp_version_label_selects_first() -> None:
    """Use the first entry when the label lists multiple OCP versions."""
    with patch(
        f"{TASK}.read_openshift_version_label",
        return_value=["v4.20", "v4.21"],
    ):
        assert fbc.extract_component_ocp_version(_component("comp", "sha256:abc")) == "v4.20"


def test_extract_component_ocp_version_label_adds_v_prefix() -> None:
    """Prefix a bare X.Y tag from the label with v."""
    with patch(f"{TASK}.read_openshift_version_label", return_value=["4.21"]):
        assert fbc.extract_component_ocp_version(_component("comp", "sha256:abc")) == "v4.21"


def test_extract_component_ocp_version_label_invalid_format() -> None:
    """Raise when the label's first version is not vX.Y."""
    with patch(f"{TASK}.read_openshift_version_label", return_value=["4.14.1"]):
        with pytest.raises(ValueError, match="Invalid OCP version format"):
            fbc.extract_component_ocp_version(_component("comp", "sha256:abc"))
