...
from release_service_utils.helpers.ocp_version import (
...
def extract_component_ocp_version(component: dict[str, Any]) -> str:
    ...
    Prefer ``com.redhat.fbc.openshift.version``; when several versions are
    listed, use the first. Fall back to the base-image annotation when the
    label is absent.
    """
    name = component.get("name", "?")
    container_image = component["containerImage"]
    logger.info("Extracting OCP version for %s from %s", name, container_image)
    logger.info('Extracting OCP version from "%s" label', FBC_OPENSHIFT_VERSION_LABEL)
    label_versions = read_openshift_version_label(container_image)
    if label_versions is not None:
        raw = label_versions[0]
        if len(label_versions) > 1:
            logger.info(
                "Multiple versions found in label (%s). Selecting first: %s",
                json.dumps(label_versions, separators=(",", ":")),
                raw,
            )
        logger.info(
            'OCP version extracted from label "%s": %s',
            FBC_OPENSHIFT_VERSION_LABEL,
            raw,
        )
    else:
        logger.info("Extracting OCP version from base image...")
        raw = resolve_ocp_version(container_image)
        if not raw:
            raise ValueError(
                f"Could not extract OCP version from {container_image}: "
                f'label "{FBC_OPENSHIFT_VERSION_LABEL}" not found; '
                "annotation 'org.opencontainers.image.base.name' not found"
            )
        logger.info("OCP version extracted from annotation: %s", raw)
