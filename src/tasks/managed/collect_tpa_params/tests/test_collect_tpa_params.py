...
class TestMain:
    ...
    def test_main_with_tsf_config(
        ...
        with patch("collect_tpa_params.collect_tpa_params.kubectl.get_configmap", mock_cm):
    ...
    def test_main_with_data_file(
        ...
        with patch(
            "collect_tpa_params.collect_tpa_params.kubectl.get_configmap",
            _mock_configmap_failure(),
        ):
...
class TestModuleMain:
    ...
    def test_module_main_guard(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with pytest.raises(SystemExit) as exc_info:
            runpy.run_module(
                "release_service_utils.tasks.managed" ".collect_tpa_params.collect_tpa_params",
                run_name="__main__",
            )
