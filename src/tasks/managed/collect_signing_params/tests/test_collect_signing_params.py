...
from release_service_utils.tasks.managed import collect_signing_params

TASK = "release_service_utils.tasks.managed.collect_signing_params"
...
def test_collect_signing_params_with_configmap(tmp_path: Path) -> None:
    ...
    with patch(
        f"{TASK}.collect_signing_params.kubectl.get_configmap", return_value=mock_configmap
    ) as m:
...
def test_collect_signing_params_missing_configmap(tmp_path: Path) -> None:
    ...
    result_paths = _make_result_paths(tmp_path)
    with patch(
        f"{TASK}.collect_signing_params.kubectl.get_configmap",
...
def test_help_via_sysargv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(SystemExit) as exc_info:
        runpy.run_module("collect_signing_params.collect_signing_params", run_name="__main__")
...
def test_module_main_guard(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with patch(
        ...
        ".collect_signing_params.collect_signing_params.kubectl.get_configmap",
