...
from release_service_utils.tasks.managed import create_pyxis_image as create_pyxis_image_pkg

create_pyxis_image = create_pyxis_image_pkg.create_pyxis_image

TASK = "release_service_utils.tasks.managed.create_pyxis_image.create_pyxis_image"
...
class TestTryPullDockerfile:
    ...
    def test_pull_ok_but_no_dockerfile_raises(self) -> None:
        ...
        with patch(f"{TASK}.oras_utils.oras_pull"):
...
class TestDecompressGzipLayer:
    ...
    def test_decompresses_and_measures(self, tmp_path: Path) -> None:
        ...
        with patch(
            f"{TASK}.oras_utils.oras_blob_fetch",
...
class TestProcessComponent:
    ...
    @patch(f"{TASK}.create_or_update")
    @patch(f"{TASK}.oras_utils.oras_manifest_fetch")
    @patch(
        f"{TASK}._get_image_architectures",
        return_value=[
            {
    ...
    @patch(
        f"{TASK}.create_or_update",
        return_value="imglayer",
    )
    @patch(f"{TASK}._decompress_gzip_layer")
    @patch(f"{TASK}.oras_utils.oras_manifest_fetch")
    @patch(
        f"{TASK}._get_image_architectures",
        return_value=[
...
class TestRun:
    ...
    @patch(f"{TASK}.process_component")
