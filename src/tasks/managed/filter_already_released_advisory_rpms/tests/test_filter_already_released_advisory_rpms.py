...
class TestExtractRpmMetadata:
    ...
    def test_success(self, tmp_path: Path) -> None:
        ...
        with patch.object(
            filt.filter_already_released_advisory_rpms.subprocess_cmd, "run_cmd", mock_run
        ):
...
class TestBuildPurl:
    ...
    def test_full_purl(self) -> None:
        ...
        result = filt.filter_already_released_advisory_rpms._build_purl(
            "hello", "1.0", "1.el9", "x86_64", "el9", "repo-1"
        )
        assert (
            result
...
class TestBuildRpmEntries:
    ...
    def _run_build(
        ...
        with (
            ...
            patch.object(
                filt.filter_already_released_advisory_rpms.file_helper,
                "sha256",
    ...
    def test_non_rpm_files_and_directories_skipped(self) -> None:
        ...
        with (
            patch.object(
                filt.filter_already_released_advisory_rpms.subprocess_cmd,
                "run_cmd",
                MagicMock(side_effect=mock_run),
            ),
            patch.object(
                filt.filter_already_released_advisory_rpms.file_helper,
                "sha256",
...
class TestValidatePulpDigests:
    ...
    def test_all_match(self) -> None:
        ...
        pulp = MagicMock(spec=filt.filter_already_released_advisory_rpms.PulpClient)
        pulp.check_digest.return_value = (
            filt.filter_already_released_advisory_rpms.PulpDigestStatus.MATCH
        )
    ...
    def test_mismatch_raises(self) -> None:
        ...
        pulp = MagicMock(spec=filt.filter_already_released_advisory_rpms.PulpClient)
        pulp.check_digest.return_value = (
            filt.filter_already_released_advisory_rpms.PulpDigestStatus.MISMATCH
        )
    ...
    def test_not_found_is_ok(self) -> None:
        ...
        pulp = MagicMock(spec=filt.filter_already_released_advisory_rpms.PulpClient)
        pulp.check_digest.return_value = (
            filt.filter_already_released_advisory_rpms.PulpDigestStatus.NOT_FOUND
        )
        filt.validate_pulp_digests([self._rpm_entry()], pulp)
...
class TestRun:
    ...
    def test_all_released_skip(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch.object(
                filt.filter_already_released_advisory_rpms,
                "make_pulp_client",
    ...
    def test_in_advisory_rpms_validated(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch.object(
                filt.filter_already_released_advisory_rpms, "validate_pulp_digests"
            ) as mock_validate,
        ):
            filt.run(cfg, res)
...
class TestMakePulpClient:
    ...
    def test_builds_client_with_basic_auth(self, tmp_path: Path) -> None:
        ...
        with (
            patch.object(
                filt.filter_already_released_advisory_rpms.http_client,
                "get_retry_session",
        ...
        assert isinstance(client, filt.filter_already_released_advisory_rpms.PulpClient)
    ...
    def test_dunder_main_block(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        ...
        with pytest.raises(SystemExit) as exc_info:
            ...
            runpy.run_module(
                ...
                ".filter_already_released_advisory_rpms.filter_already_released_advisory_rpms",
                run_name="__main__",
            )
