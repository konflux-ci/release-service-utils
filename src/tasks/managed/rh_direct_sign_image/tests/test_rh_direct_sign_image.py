...
TASK = "release_service_utils.tasks.managed.rh_direct_sign_image.rh_direct_sign_image"
...
@patch(f"{TASK}.pyxis.graphql_query")
...
def test_filter_already_signed_returns_all_when_nothing_signed() -> None:
    ...
    with patch(f"{TASK}.find_existing_signatures", return_value={}):
...
def test_filter_already_signed_removes_already_signed_item() -> None:
    ...
    with patch(
        f"{TASK}.find_existing_signatures",
...
def test_filter_already_signed_source_container_items_filtered() -> None:
    ...
    with patch(f"{TASK}.find_existing_signatures", return_value=existing):
        result = filter_already_signed(items, PYXIS_URL)
...
def test_filter_already_signed_passes_exact_pairs_to_pyxis() -> None:
    ...
    with patch(f"{TASK}.find_existing_signatures", return_value={}) as mock_find:
...
def test_process_component_returns_all_candidates() -> None:
    ...
    with (
        patch(
            f"{TASK}.get_all_image_digests",
            return_value=["sha256:abc"],
        ),
        patch(
            f"{TASK}.get_source_container_digest",
            return_value=None,
        ),
    ):
...
def test_get_source_container_digest_returns_none_when_disabled() -> None:
    ...
    component = {
        "containerImage": "registry.io/repo@sha256:abc",
...
def test_get_source_container_digest_resolves_digest() -> None:
    ...
    with patch(f"{TASK}.oras_resolve", return_value="sha256:sourceabc"):
        result = get_source_container_digest(component, default_push_source_container=False)
...
def test_get_source_container_digest_constructs_correct_source_reference() -> None:
    ...
    with patch(f"{TASK}.oras_resolve", return_value="sha256:src") as mock_resolve:
...
def test_find_existing_signatures_queries_each_pair() -> None:
    ...
    with patch(f"{TASK}.find_signatures_for_repository", side_effect=[{sig_a}, set()]):
...
def test_find_existing_signatures_returns_empty_set_for_missing_pair() -> None:
    ...
    with patch(f"{TASK}.find_signatures_for_repository", return_value=set()):
        results = find_existing_signatures(PYXIS_URL, lookups)
...
def test_main_collects_candidates_and_writes_batches(tmp_path) -> None:
    ...
    with (
        patch(f"{TASK}.pyxis._get_session"),
        patch(
            f"{TASK}.get_configmap",
            return_value={"data": {"SIG_KEY_NAME": "key-a"}},
        ),
        patch(f"{TASK}.process_component", return_value=[item]),
        patch(f"{TASK}.filter_already_signed", return_value=[item]),
        patch(
            "sys.argv",
            [
...
def test_main_returns_zero_on_success(tmp_path) -> None:
    ...
    with (
        ...
        patch(f"{TASK}.filter_already_signed", return_value=[]),
        patch(
            "sys.argv",
            [
                "prepare_container_signing",
...
def test_submit_batches_submits_each_batch_file(tmp_path) -> None:
    ...
    with patch(f"{TASK}.submit_batch") as mock_submit:
...
def test_submit_batches_uses_concurrent_limit(tmp_path) -> None:
    ...
    with (
        patch(f"{TASK}.submit_batch"),
        patch(f"{TASK}.ThreadPoolExecutor") as mock_pool_cls,
        patch(f"{TASK}.as_completed", return_value=[]),
    ):
        mock_pool = MagicMock()
...
def test_submit_batches_logs_all_succeeded(tmp_path) -> None:
    ...
    with (
        patch(f"{TASK}.submit_batch"),
        patch(f"{TASK}.LOGGER") as mock_logger,
    ):
        submit_batches(tmp_path, cfg)
...
def test_submit_batches_logs_partial_failure_counts(tmp_path) -> None:
    ...
    with (
        patch(f"{TASK}.submit_batch", side_effect=fail_first),
        patch(f"{TASK}.LOGGER") as mock_logger,
...
def test_submit_batches_raises_after_logging_summary(tmp_path) -> None:
    ...
    with (
        patch(f"{TASK}.submit_batch", side_effect=RuntimeError("request failed")),
...
def test_main_submits_batches_when_flag_set(tmp_path) -> None:
    ...
    with (
        patch(f"{TASK}.pyxis._get_session"),
        patch(f"{TASK}.get_configmap", return_value=_FULL_CONFIGMAP),
        patch(
            f"{TASK}.filter_already_signed",
            return_value=[
                SigningItem("registry.redhat.io/repo:tag", "sha256:abc", "repo", "key-a"),
            ],
        ),
        patch(f"{TASK}.submit_batches") as mock_submit,
        patch(
            "sys.argv",
            [
...
def test_main_always_writes_batches(tmp_path) -> None:
    ...
    with (
        patch(f"{TASK}.pyxis._get_session"),
        patch(f"{TASK}.get_configmap", return_value={"data": {"SIG_KEY_NAME": "key-a"}}),
        patch(
            f"{TASK}.filter_already_signed",
            return_value=[
                SigningItem("registry.redhat.io/repo:tag", "sha256:abc", "repo", "key-a"),
        ...
        patch(f"{TASK}.write_batches") as mock_write,
        patch(
            "sys.argv",
            [
                "prepare_container_signing",
