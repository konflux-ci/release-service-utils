...
class TestGroupComponentsByOcpVersion:
            ...
            "components": [
                {
                    ...
                    "ocpVersionMetadata": [
                        {
                            "version": "v4.17",
                            "updatedFromIndex": "quay.io/idx:v4.17",
                            "targetIndex": "quay.io/target:v4.17",
                        },
                        {
                            "version": "v4.18",
                            "updatedFromIndex": "quay.io/idx:v4.18",
                            "targetIndex": "quay.io/target:v4.18",
                        },
                    ],
                }
    ...
    def test_target_index_without_tag_raises_error(self) -> None:
        """ValueError is raised when targetIndex has no tag."""
        snapshot = {
            "components": [
                {
                    "name": "comp1",
                    "containerImage": "registry.io/comp1@sha256:0001",
                    "ocpVersion": "4.12",
                    "updatedFromIndex": "quay.io/from:latest",
                    "targetIndex": "quay.io/target-no-tag",
                }
            ]
        }
        ocp_versions = ["4.12"]
        ...
        with pytest.raises(ValueError, match="must include a tag"):
            group_components_by_ocp_version(snapshot, ocp_versions, [])
...
class TestComputeTargetIndexWithTimestamp:
    ...
    def test_target_index_ending_with_10_digits_no_hyphen(self) -> None:
        """Target index ending with 10 digits (no hyphen) is treated as timestamped."""
        result = compute_target_index_with_timestamp("quay.io/target:1234567890", "1709746751")

        assert result == "quay.io/target:1234567890"

    def test_target_index_with_14_digit_timestamp(self) -> None:
        """Target index with 14-digit timestamp (YYYYMMDDHHmmss) is treated as timestamped."""
        result = compute_target_index_with_timestamp(
            "quay.io/target:v4.17-20240306163911", "1709746751"
        )

        assert result == "quay.io/target:v4.17-20240306163911"

    def test_target_index_with_short_digits_gets_timestamp(self) -> None:
        """Target index ending with fewer than 10 digits gets timestamp appended."""
        result = compute_target_index_with_timestamp("quay.io/target:v4.17-123", "1709746751")

        assert result == "quay.io/target:v4.17-123-1709746751"
...
class TestProcessBatchResults:
    ...
    def test_processes_successful_batch(self, tmp_path: Path) -> None:
        ...
        build_info = {
            ...
            "internal_index_image_copy_resolved": "quay.io/iib@sha256:abc",
        }
        batch_result = BatchResult(
            ...
            index_image_digests=["sha256:a", "sha256:b"],
            iib_log="Test log",
        )
...
class TestExecuteBatch:
    ...
    def test_nonzero_exit_code_in_results(self, tmp_path: Path, monkeypatch: Any) -> None:
        """InternalRequest with non-zero exitCode is treated as failure."""
        config = make_config(tmp_path)
        group = OCPGroup(
            ocp_version="4.12",
            components=[make_component("comp1", "4.12", container_image="img1")],
        ...
        mock_fetch = mock.MagicMock(
            return_value={
                "exitCode": "1",
                "resultMessage": "IIB build failed: invalid fragment",
            }
        )
        ...
        assert batch_result.success is False
        assert "exit code 1" in batch_result.error_message
        assert "IIB build failed" in batch_result.error_message
...
class TestMain:
    ...
    def test_missing_required_env_vars(self, monkeypatch: Any) -> None:
        """Missing required environment variables cause error."""
    ...
    def test_main_writes_result_files(self, tmp_path: Path, monkeypatch: Any) -> None:
        ...
        monkeypatch.setenv("PIPELINE_RUN_UID", "test-plr")
        monkeypatch.setenv("TASK_RUN_UID", "test-tr")
        ...
        monkeypatch.setenv("IIB_SERVICE_ACCOUNT_SECRET", "test-secret")
        monkeypatch.setenv("MUST_PUBLISH_INDEX_IMAGE", "true")
        monkeypatch.setenv("MUST_OVERWRITE_FROM_INDEX_IMAGE", "true")
        monkeypatch.setenv("RESULT_BUILD_TIMESTAMP", str(build_timestamp_result))
        monkeypatch.setenv("RESULT_REQUEST_RESULTS_FILE", str(request_results_result))
        monkeypatch.setenv("RESULT_INTERNAL_REQUEST_RESULTS_FILE", str(ir_results_result))
        ...
        request_results_content = request_results_result.read_text()
        assert request_results_content == str(tmp_path / "test-plr" / "ir-test-tr-result.json")

        ir_results_content = ir_results_result.read_text()
        assert ir_results_content == "results/internal-requests-results.json"
...
class TestEdgeCases:
    ...
    def test_process_batch_results_empty_build_info(self, tmp_path: Path) -> None:
        """Skips processing when build_info is empty."""
        config = make_config(tmp_path)
        group = OCPGroup(
            ocp_version="4.12",
            components=[make_component("comp1", "4.12", container_image="img1")],
    ...
    def test_ocp_group_updates_from_index_when_not_overwrite(
        ...
        build_info_1 = {
            ...
            "internal_index_image_copy_resolved": "quay.io/iib@sha256:abc1",
        }
        build_info_2 = {
            "updated": "2024-03-06T16:40:11Z",
            "index_image": "quay.io/iib:batch2",
            "internal_index_image_copy_resolved": "quay.io/iib@sha256:abc2",
        }
    ...
    def test_ocp_group_retry_failed_batches_at_end(
        self, tmp_path: Path, monkeypatch: Any
    ) -> None:
    ...
    def test_ocp_group_updates_from_index_after_successful_batch(
        ...
        build_info_1 = {
            "updated": "2024-03-06T16:39:11Z",
            "index_image": "quay.io/iib:batch1",
            "internal_index_image_copy_resolved": "quay.io/iib@sha256:batch1",
        }
        build_info_2 = {
            "updated": "2024-03-06T16:40:11Z",
            "index_image": "quay.io/iib:batch2",
            "internal_index_image_copy_resolved": "quay.io/iib@sha256:batch2",
        }
    ...
    def test_invalid_completion_time_triggers_retry(
        self, tmp_path: Path, monkeypatch: Any
    ) -> None:
        """Batch with invalid completion_time is treated as failed and retried."""
        config = make_config(
            tmp_path,
            max_batch_size=1,
            max_retries=1,
        ...
        valid_build_info = {
            "updated": "2024-03-06T16:39:11Z",
            "index_image": "quay.io/iib:02",
            "internal_index_image_copy_resolved": "quay.io/iib@sha256:def",
        }

        fetch_results = [
            {
                "jsonBuildInfo": iib.compress_build_info(invalid_build_info),
                "indexImageDigests": "sha256:a",
            },
            {
                "jsonBuildInfo": iib.compress_build_info(valid_build_info),
                "indexImageDigests": "sha256:b",
            },
        ]
        fetch_call_count = [0]
