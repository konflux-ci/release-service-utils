...
def get_signing_key_name(config_map_name: str) -> str:
    ...
    Prefers the comma-separated ``SIG_KEY_NAMES`` field, using its first
    entry, matching the precedence used by other signing consumers (e.g.
    rh-sign-image/sign-index-image). Falls back to the singular
    ``SIG_KEY_NAME`` field when ``SIG_KEY_NAMES`` is absent or empty.
    ...
    *,
    ...
    try:
        ir_name = internal_request.create(
            ...
            labels={internal_request.PIPELINERUN_UID_LABEL: pipeline_run_uid},
            sync=True,
