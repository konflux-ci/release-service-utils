"""Test push_artifacts_to_cdn module."""
...
class TestGetSigningKeyName:
    ...
    def test_prefers_plural_first_entry(self, mock_get_configmap: MagicMock) -> None:
        """The first entry of SIG_KEY_NAMES is preferred when both fields are present."""
        mock_get_configmap.return_value = {
            "data": {"SIG_KEY_NAME": "singleKey", "SIG_KEY_NAMES": "a,b"}
        }
        assert get_signing_key_name("test-config-map") == "a"

    @patch(f"{TASK}.kubectl.get_configmap")
    def test_falls_back_to_singular_key(self, mock_get_configmap: MagicMock) -> None:
        """SIG_KEY_NAME is used when SIG_KEY_NAMES is absent."""
        mock_get_configmap.return_value = {"data": {"SIG_KEY_NAME": "singleKey"}}
