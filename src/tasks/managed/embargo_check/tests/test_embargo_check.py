...
from release_service_utils.tasks.managed import embargo_check
...
class TestGetContentItems:
    ...
    def test_returns_images(self) -> None:
        ...
        assert embargo_check.embargo_check._get_content_items(data) == [{"name": "img1"}]
    ...
    def test_returns_artifacts(self) -> None:
        ...
        assert embargo_check.embargo_check._get_content_items(data) == [{"name": "art1"}]
    ...
    def test_returns_empty_list_for_empty_content(self) -> None:
        ...
        assert embargo_check.embargo_check._get_content_items(data) == []

    def test_returns_none(self) -> None:
        """Return None when no content exists."""
        assert embargo_check.embargo_check._get_content_items({}) is None
        assert embargo_check.embargo_check._get_content_items({"releaseNotes": {}}) is None
        assert (
            embargo_check.embargo_check._get_content_items({"releaseNotes": {"content": {}}})
            is None
        )
...
class TestExtractCves:
    ...
    def test_extracts_cves_from_images(self) -> None:
        ...
        assert embargo_check.embargo_check._extract_cves(data) == [
            "CVE-2024-001",
            "CVE-2024-002",
        ]
    ...
    def test_extracts_cves_from_artifacts(self) -> None:
        ...
        assert embargo_check.embargo_check._extract_cves(data) == ["CVE-2024-001"]

    def test_returns_empty_no_content(self) -> None:
        """Return empty list when no content exists."""
        assert embargo_check.embargo_check._extract_cves({}) == []
...
class TestCheckIssueVisibility:
    ...
    def test_returns_true_on_success(self) -> None:
        ...
        assert (
            embargo_check.embargo_check._check_issue_visibility(
...
class TestGetWithJira404Retry:
    ...
    @mock.patch("release_service_utils.tasks.managed.embargo_check.embargo_check.time.sleep")
    def test_retries_on_transient_404(self, mock_sleep: mock.MagicMock) -> None:
        ...
        result = embargo_check.embargo_check._get_with_jira_404_retry(
    ...
    def test_raises_after_exhausting_404_retries(self, mock_sleep: mock.MagicMock) -> None:
        ...
        with pytest.raises(requests.HTTPError):
            embargo_check.embargo_check._get_with_jira_404_retry(
                session, "https://jira.test/issue/X-1", auth
            )
        assert session.get.call_count == embargo_check.embargo_check.MAX_JIRA_404_RETRIES
...
class TestFormatTimeout:
    ...
    def test_formats_seconds_only(self) -> None:
        ...
        assert embargo_check.embargo_check._format_timeout(45) == "00h00m45s"

    def test_formats_minutes_and_seconds(self) -> None:
        """Format 3000 seconds as 00h50m00s."""
        assert embargo_check.embargo_check._format_timeout(3000) == "00h50m00s"

    def test_formats_hours(self) -> None:
        """Format 3661 seconds as 01h01m01s."""
        assert embargo_check.embargo_check._format_timeout(3661) == "01h01m01s"
...
class TestCheckIssues:
    ...
    def test_no_issues(self, jira_secret_path: Path) -> None:
        ...
        session = mock.MagicMock(spec=requests.Session)
        result_data, errors = embargo_check.embargo_check.check_issues(
...
class TestCheckCves:
    ...
    @mock.patch("release_service_utils.tasks.managed.embargo_check.embargo_check.create")
...
class TestRun:
    ...
    @mock.patch(
        "release_service_utils.tasks.managed.embargo_check.embargo_check.check_cves",
        return_value=[],
    )
    @mock.patch("release_service_utils.tasks.managed.embargo_check.embargo_check.check_issues")
    @mock.patch("release_service_utils.helpers.http_client.get_retry_session")
    def test_success(
        self,
    ...
    @mock.patch("release_service_utils.tasks.managed.embargo_check.embargo_check.check_cves")
    @mock.patch("release_service_utils.tasks.managed.embargo_check.embargo_check.check_issues")
    @mock.patch("release_service_utils.helpers.http_client.get_retry_session")
    ...
    def test_raises_on_cve_errors(
        ...
        with pytest.raises(RuntimeError, match="CVE error"):
            embargo_check.embargo_check.run(
                tmp_path / "data.json",
                secret_path=tmp_path / "secrets",
                pipeline_run_uid="uid",
                request_timeout=2700,
...
class TestMain:
    ...
    def test_success(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with mock.patch(
            "release_service_utils.tasks.managed.embargo_check.embargo_check.run"
        ) as mock_run:
            assert embargo_check.embargo_check.main() == 0
    ...
    def test_missing_required_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with pytest.raises(SystemExit):
            embargo_check.embargo_check.main()
    ...
    def test_module_main_guard(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with mock.patch("release_service_utils.tasks.managed.embargo_check.run"):
            with pytest.raises(SystemExit) as exc_info:
                runpy.run_module(
                    "release_service_utils.tasks.managed.embargo_check.embargo_check",
                    run_name="__main__",
                )
