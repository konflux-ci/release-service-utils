...
from release_service_utils.tasks.managed.direct_sign_index_image import (
    collect_fbc_signing_items,
    main,
    setup_argparser,
    translate_reference,
)
from release_service_utils.tasks.managed.rh_direct_sign_image import SigningItem

TASK = "release_service_utils.tasks.managed.direct_sign_index_image.direct_sign_index_image"
...
@patch(f"{TASK}.translate_reference")
...
def test_main_happy_path_writes_batches_and_submits(tmp_path) -> None:
    ...
    with (
        patch(f"{TASK}.get_configmap", return_value=CONFIGMAP),
        patch(f"{TASK}.collect_fbc_signing_items", return_value=[item]),
...
def test_main_pyxis_failure_with_fail_on_error_false_submits_all(tmp_path) -> None:
    ...
    with (
        patch(f"{TASK}.get_configmap", return_value=CONFIGMAP),
        patch(f"{TASK}.collect_fbc_signing_items", return_value=items),
        patch(
            f"{TASK}.filter_already_signed",
            side_effect=RuntimeError("Pyxis connection failed"),
        ),
...
def test_main_uses_configmap_name_from_data_file(tmp_path) -> None:
    ...
    with (
        patch(f"{TASK}.get_configmap", return_value=CONFIGMAP) as mock_cm,
        patch("sys.argv", _base_argv(fbc_path, data_path, output_dir)),
    ):
        main()
...
def test_main_passes_max_workers_to_filter(tmp_path) -> None:
    ...
    with (
        patch(f"{TASK}.get_configmap", return_value=CONFIGMAP),
        patch(f"{TASK}.collect_fbc_signing_items", return_value=[item]),
        patch(f"{TASK}.filter_already_signed", return_value=[item]) as mock_filter,
        patch(f"{TASK}.submit_batches"),
        patch(
            "sys.argv",
...
def test_main_translate_failure_raises(tmp_path) -> None:
    ...
    with (
        patch(f"{TASK}.get_configmap", return_value=CONFIGMAP),
        patch(
            f"{TASK}.collect_fbc_signing_items",
