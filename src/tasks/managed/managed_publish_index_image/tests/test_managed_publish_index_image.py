...
@pytest.fixture
def fix_cleanup_existing_requests():
    """Mock _cleanup_existing_requests to do nothing."""
...
@pytest.fixture
def fix_wait_failed():
    """Mock wait_for_internal_request to raise InternalRequestWaitError."""
    ...
    with mock.patch.object(
        ...
        def create_mock_future(*args, **kwargs):
            future = Future()
            future.set_exception(InternalRequestWaitError("IR failed", 21))
            return future
...
class TestCreateIrWrapper:
    ...
    def test_create_ir_wrapper_with_labels(self) -> None:
        ...
        with (
            mock.patch.object(
                managed_publish_index_image.managed_publish_index_image, "_build_payload"
            ...
            mock_build_payload.return_value = {"test": "payload"}
            ...
            mock_build_payload.assert_called_once_with(
                ...
                labels={"label1": "val1", "label2": "val2"},
                pipeline_git_url="url",
                pipeline_git_revision="rev",
                pipeline_timeout="2h",
                task_timeout="1h",
                finally_timeout="0h5m0s",
                service_account=None,
            )
            mock_create_ir.assert_called_once_with({"test": "payload"})
    ...
    def test_create_ir_wrapper_without_labels(self) -> None:
        ...
        with (
            ...
            params = [("key1", "value1"), ("taskGitUrl", "url"), ("taskGitRevision", "rev")]
            labels = None
