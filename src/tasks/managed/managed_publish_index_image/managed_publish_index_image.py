...
from release_service_utils.helpers.internal_request import (
    ...
    cleanup_existing_requests as _cleanup_existing_requests,
...
def _create_ir_wrapper(pipeline, params, labels, task_timeout, pipeline_timeout, **kwargs):
    """Wrap build_payload + create_internal_request for async executor.
    ...
    payload = _build_payload(
        ...
        params=params_dict,
        labels=labels_dict,
        pipeline_git_url=params_dict["taskGitUrl"],
        pipeline_git_revision=params_dict["taskGitRevision"],
    ...
    return _create_internal_request(payload)
...
def main() -> int:
    ...
    # Cleanup existing IRs once before creating any new ones
    cleanup_labels = {PIPELINERUN_LABEL: args.pipeline_run_id}
    _cleanup_existing_requests(
    ...
    for ft in as_completed(wait_futures):
        ...
        try:
            ...
            # Timeout or failed IR - treat as failure
            failed = True
            results = _fetch_results(ir_name)
            request_message = results.get("requestMessage", "")
            log.error(f"ERROR: InternalRequest {ir_name} timed out or failed: {e}")
        ...
        log.debug(f"requestMessage: {request_message}")
