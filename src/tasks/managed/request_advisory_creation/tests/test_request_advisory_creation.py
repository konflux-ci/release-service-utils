...
def test_prepare_advisory_data_defaults_missing_type_to_rhba() -> None:
    ...
    out = request_advisory_creation.request_advisory_creation._prepare_advisory_data(
        advisory,
        ".content.images",
    )
...
def test_prepare_advisory_data_rejects_rhsa_without_cves() -> None:
    ...
    with pytest.raises(ValueError, match="no fixed CVEs were listed"):
        request_advisory_creation.request_advisory_creation._prepare_advisory_data(
...
def test_resolve_content_type_uses_github_generic_default() -> None:
    ...
    assert (
        request_advisory_creation.request_advisory_creation._resolve_content_type(
            {"github": {}}
        )
        == "generic"
    )
...
def test_run_request_advisory_creation_calls_update_purl_before_internal_request(
    ...
    with (
        ...
        mock.patch.object(
            request_advisory_creation.request_advisory_creation,
...
def test_create_internal_request_calls_python_helper() -> None:
    ...
    with mock.patch.object(internal_request, "create", return_value="ir-abc") as ir:
        name = request_advisory_creation.request_advisory_creation._create_internal_request(
            params,
            component_group="grp",
            origin="origin-ws",
    ...
    ir.assert_called_once_with(
        ...
        sync=True,
        timeout=(request_advisory_creation.request_advisory_creation._IR_WAIT_TIMEOUT_SECONDS),
        pipeline_timeout=(
            request_advisory_creation.request_advisory_creation._IR_PIPELINE_TIMEOUT
        ),
        task_timeout=(request_advisory_creation.request_advisory_creation._IR_TASK_TIMEOUT),
        finally_timeout=(
            request_advisory_creation.request_advisory_creation._IR_FINALLY_TIMEOUT
        ),
    )
...
def test_main_success(tmp_path: Path) -> None:
    ...
    with (
        ...
        assert request_advisory_creation.request_advisory_creation.main() == 0
...
def test_content_path_for_type_uses_artifacts_for_binary() -> None:
    ...
    assert (
        request_advisory_creation.request_advisory_creation._content_path_for_type("binary")
        == ".content.artifacts"
    )
...
def test_count_fixed_cves_counts_list_fixed_cves() -> None:
    ...
    assert (
        request_advisory_creation.request_advisory_creation._count_fixed_cves(
            advisory, ".content.images"
        )
        == 2
    )
...
def test_resolve_secret_names_image_uses_staging_secrets() -> None:
    ...
    assert request_advisory_creation.request_advisory_creation._resolve_secret_names(
        "image",
        environment="staging",
        data={},
    ) == (
...
def test_resolve_secret_names_binary_uses_intention() -> None:
    ...
    prod = request_advisory_creation.request_advisory_creation._resolve_secret_names(
        "binary",
        environment="",
        data={"intention": "production"},
    )
    staging = request_advisory_creation.request_advisory_creation._resolve_secret_names(
        "binary",
        environment="",
...
def test_resolve_secret_names_rejects_unsupported_intention() -> None:
    ...
    with pytest.raises(ValueError, match="unsupported intention"):
        request_advisory_creation.request_advisory_creation._resolve_secret_names(
            "binary",
            environment="",
            data={"intention": "dev"},
        )
...
def test_sync_from_param_parses_false() -> None:
    ...
    assert (
        request_advisory_creation.request_advisory_creation._sync_from_param("false") is False
    )
...
def test_create_internal_request_wraps_wait_error(tmp_path: Path) -> None:
    ...
    with (
        mock.patch.object(
            internal_request,
            "create",
            side_effect=internal_request.InternalRequestWaitError(
                "timed out",
                1,
            ),
        ),
        ...
        request_advisory_creation.request_advisory_creation._create_internal_request(
            params,
            component_group="grp",
            origin="origin-ws",
            advisory_json="encoded",
...
def test_write_task_results_raises_on_failure(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="advisory creation failed"):
        request_advisory_creation.request_advisory_creation._write_task_results(
            params,
...
def test_ir_wait_timeout_covers_pipeline_budget() -> None:
    ...
    assert request_advisory_creation.request_advisory_creation._IR_WAIT_TIMEOUT_SECONDS == (
...
def test_params_from_env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    params = request_advisory_creation.request_advisory_creation._params_from_env()
