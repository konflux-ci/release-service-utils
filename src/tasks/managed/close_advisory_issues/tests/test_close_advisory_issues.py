...
def test_close_issue_with_comment_posts_transition_payload() -> None:
    ...
    session = mock.MagicMock()
    auth = HTTPBasicAuth("user", "token")
    with mock.patch("close_advisory_issues.close_advisory_issues.jira_post_json") as post_json:
...
def test_close_advisory_issues_skips_jira_setup_without_eligible_issues(
    ...
    with mock.patch(
        "close_advisory_issues.close_advisory_issues.read_jira_credentials"
    ) as read_creds:
        with mock.patch(
            "close_advisory_issues.close_advisory_issues.http_client.get_retry_session",
        ) as create_session:
...
def test_close_advisory_issues_creates_retry_session_when_jira_issue_present(
    ...
    with mock.patch(
        ...
        with mock.patch(
            "close_advisory_issues.close_advisory_issues.process_fixed_issue"
        ) as process_issue:
            close_advisory_issues.close_advisory_issues(
                data_dir=tmp_path,
                data_path=Path("data.json"),
...
def test_process_fixed_issue_skips_already_closed_issue(
    ...
    auth = HTTPBasicAuth("user", "token")
    with mock.patch(
        "close_advisory_issues.close_advisory_issues.jira_get_json",
...
def test_process_fixed_issue_closes_open_issue() -> None:
    ...
    with mock.patch(
        ...
        with mock.patch(
            "close_advisory_issues.close_advisory_issues.close_issue_with_comment",
        ) as close_issue:
            close_advisory_issues.process_fixed_issue(
                _fixed_issue("ISSUE-123"),
                advisory_url="https://access.redhat.com/errata/RHBA-2025:1111",
...
def test_process_fixed_issue_adds_comment_when_no_closed_transition(
    ...
    with mock.patch(
        ...
        with mock.patch(
            "close_advisory_issues.close_advisory_issues.add_issue_comment"
        ) as add_comment:
            with caplog.at_level(logging.WARNING, logger="release"):
                close_advisory_issues.process_fixed_issue(
...
def test_main_success(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with mock.patch(
        "close_advisory_issues.close_advisory_issues.close_advisory_issues"
    ) as run:
