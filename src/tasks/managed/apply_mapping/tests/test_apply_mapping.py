...
def test_process_component_jinja_substitution_label_with_dashes() -> None:
    """Labels with dashes in their names are substituted correctly."""
    component = _base_component()
    component["unstableFields"] = {"customLabel": "{{ labels.label-with-dashes }}"}
    raw_manifest = {
        "config": {
            "mediaType": "application/vnd.oci.image.config.v1+json",
            "Labels": {"label-with-dashes": "value-from-label"},
        }
    }
    standard = {"Labels": {"label-with-dashes": "value-from-label"}}
    apply_mapping.process_component(
        component,
        default_tags=[],
        default_timestamp_format="%s",
    ...
    assert component["unstableFields"]["customLabel"] == "value-from-label"
