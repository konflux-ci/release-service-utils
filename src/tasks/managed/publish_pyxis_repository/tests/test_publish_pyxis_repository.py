...
def test_publish_repositories_happy_path(tmp_path: Path) -> None:
    ...
    with (
        mock.patch(
            "publish_pyxis_repository.publish_pyxis_repository.pyxis_api.get_repository_json",
            side_effect=fake_get,
        ),
        mock.patch(
            "publish_pyxis_repository.publish_pyxis_repository"
...
def test_run_publish_pyxis_repository_writes_outputs(tmp_path: Path) -> None:
    ...
    with (
        ...
        mock.patch(
            "publish_pyxis_repository.publish_pyxis_repository.pyxis_api"
...
def test_module_main_guard_raises_system_exit(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(SystemExit) as exc:
        runpy.run_path(
            str(Path(publish_pyxis_repository.publish_pyxis_repository.__file__)),
            run_name="__main__",
        )
...
def test_main_unexpected_failure_propagates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with mock.patch(
        "publish_pyxis_repository.publish_pyxis_repository.run_publish_pyxis_repository",
