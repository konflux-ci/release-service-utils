...
from release_service_utils.tasks.managed.extract_oot_kmods.extract_oot_kmods import (
    _extract_layer,
    _ko_prefix,
    _write_summary,
    extract_single_arch,
...
TASK = "release_service_utils.tasks.managed.extract_oot_kmods.extract_oot_kmods"
...
def test_get_image_architectures_single() -> None:
    ...
    with patch(
        "release_service_utils.tasks.managed.extract_oot_kmods.extract_oot_kmods.run_cmd_text",
        return_value=ndjson,
    ):
...
def test_extract_single_arch_success(tmp_path: Path) -> None:
    ...
    with patch(
        "release_service_utils.tasks.managed.extract_oot_kmods.extract_oot_kmods.skopeo.copy",
...
def test_run_single_arch(tmp_path: Path) -> None:
    ...
    with (
        patch(
            f"{TASK}.snapshot_helper.first_component",
            return_value=_mock_first_component(),
        ),
        patch(
            f"{TASK}.get_image_architectures",
            return_value=arch_data,
        ),
        patch(
            f"{TASK}.extract_single_arch",
            return_value=True,
        ),
    ):
...
def test_run_multi_arch(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch(f"{TASK}._write_summary") as mock_summary,
    ):
        run(tmp_path, "snapshot.json", "/kmods", "signed-kmods")
...
def test_main_success(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with patch(
        "release_service_utils.tasks.managed.extract_oot_kmods.extract_oot_kmods.run"
    ) as mock_run:
        assert main() == 0
