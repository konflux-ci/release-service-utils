...
from release_service_utils.tasks.managed import update_infra_deployments as task
from release_service_utils.helpers.vcs import github

TASK = "release_service_utils.tasks.managed.update_infra_deployments"
...
def test_extract_old_revision_new_tag() -> None:
    ...
    diff = " context\n-    newTag: abc123\n+    newTag: def456\n"
    assert task.update_infra_deployments._extract_old_revision_from_diff(diff) == "abc123"


def test_extract_old_revision_ignores_version() -> None:
    """Ignore `version` field changes when extracting the old revision."""
    diff = "-    version: 1.2.3\n+    version: 1.3.0\n"
    assert task.update_infra_deployments._extract_old_revision_from_diff(diff) == ""


def test_bash_script_from_data_empty() -> None:
    """Return `None` when the legacy bash script key is missing or blank."""
    assert task.update_infra_deployments._bash_script_from_data({}) is None
    assert (
        task.update_infra_deployments._bash_script_from_data(
...
def test_sandboxed_script_from_data_empty() -> None:
    ...
    assert task.update_infra_deployments._sandboxed_script_from_data({}) is None
    assert (
        task.update_infra_deployments._sandboxed_script_from_data(
...
def test_sandboxed_script_from_data_returns_text() -> None:
    ...
    assert (
        task.update_infra_deployments._sandboxed_script_from_data(data)
        == "replace('f', r'x', 'y')"
    )
...
def test_github_app_ids() -> None:
    ...
    assert task.update_infra_deployments._github_app_ids(
        data,
        default_app_id="1",
        default_installation_id="2",
    ) == ("9", "8")
...
def test_run_update_script_prints_output(
    ...
    task.update_infra_deployments._run_update_script("echo hello\n", tmp_path)
...
def test_run_update_script_prints_stderr(
    ...
    task.update_infra_deployments._run_update_script("echo err 1>&2\n", tmp_path)
...
def test_collect_apply_result() -> None:
    ...
    with (
        mock.patch(
            f"{TASK}.update_infra_deployments.git.working_tree_diff",
            return_value="-    newTag: x\n",
        ),
        mock.patch(
            f"{TASK}.update_infra_deployments.git.changed_paths_from_status",
            return_value=["a.yaml"],
        ),
    ):
        result = task.update_infra_deployments._collect_apply_result(snap, Path("/repo"))
...
def test_build_pr_description_appends_changelog() -> None:
    ...
    with (
        mock.patch(
            f"{TASK}.update_infra_deployments.github.pull_request_url_for_commit_sha",
            return_value="https://github.com/o/r/pull/1",
        ),
        mock.patch(
            f"{TASK}.update_infra_deployments.github.compare_changelog",
            return_value="## Changelog\n- new item",
        ),
        mock.patch(
            f"{TASK}.update_infra_deployments.image_ref.resolve_quay_digest_to_git_sha",
            return_value="oldrev",
        ),
    ):
        body = task.update_infra_deployments._build_pr_description(
            session,
...
def test_merge_changelog_section_dedupes_lines() -> None:
    ...
    body = "Links\n\n## Changelog\n- same"
    merged = task.update_infra_deployments._merge_changelog_section(
        body, "## Changelog\n- same\n- other"
    )
...
def test_build_pr_description_uses_tag_old_revision() -> None:
    ...
    with (
        ...
        task.update_infra_deployments._build_pr_description(
            session,
            existing_body=None,
            origin_repo="https://github.com/org/repo",
            revision="newrev",
...
def test_snapshot_from_params(tmp_path: Path) -> None:
    ...
    snap = task.update_infra_deployments._snapshot_from_params(params)
...
def test_run_patched_script(tmp_path: Path) -> None:
    ...
    with mock.patch(f"{TASK}.update_infra_deployments._run_update_script") as run:
        task.update_infra_deployments._run_patched_script(
            'echo "{{ revision }}"', "rev123", tmp_path
        )
...
def test_create_or_update_pr_no_changed_paths() -> None:
    ...
    with mock.patch(f"{TASK}.update_infra_deployments.github.open_session") as open_session:
        task.update_infra_deployments._create_or_update_pr(
            params,
            {},
...
def test_create_or_update_pr_happy_path(tmp_path: Path) -> None:
    ...
    with (
        mock.patch(
            f"{TASK}.update_infra_deployments.github.open_session",
            return_value=session,
        ),
        mock.patch(f"{TASK}.update_infra_deployments.github.force_push_updated_files"),
        mock.patch(
            f"{TASK}.update_infra_deployments.github.create_pull_request",
            return_value={"url": "https://api.github.com/pull/1", "body": "old"},
        ),
        mock.patch(
            f"{TASK}.update_infra_deployments._build_pr_description",
            return_value="new body",
        ),
        mock.patch(
            f"{TASK}.update_infra_deployments.github.update_pull_request_body"
        ) as update,
    ):
        task.update_infra_deployments._create_or_update_pr(
            params,
...
def test_create_or_update_pr_finds_existing_when_create_missing_url(tmp_path: Path) -> None:
    ...
    with (
        ...
        mock.patch(
            f"{TASK}.update_infra_deployments.github.find_open_pull_request_by_branch",
        ...
        mock.patch(f"{TASK}.update_infra_deployments.github.update_pull_request_body"),
    ):
        task.update_infra_deployments._create_or_update_pr(
            params,
            {},
...
def test_run_update_infra_deployments_syncs_main(tmp_path: Path) -> None:
    ...
    with (
        mock.patch(
            f"{TASK}.update_infra_deployments.git.clone",
            return_value=clone_dir,
        ),
        mock.patch(f"{TASK}.update_infra_deployments.git.sync_to_origin_main") as sync,
        mock.patch.object(task.update_infra_deployments, "_run_patched_script"),
        mock.patch.object(
            task.update_infra_deployments,
        ...
        mock.patch.object(task.update_infra_deployments, "_create_or_update_pr"),
...
def test_run_update_infra_deployments_creates_work_dir(tmp_path: Path) -> None:
    ...
    with (
        ...
        mock.patch(f"{TASK}.update_infra_deployments.git.sync_to_origin_main"),
        mock.patch.object(task.update_infra_deployments, "_run_patched_script"),
        mock.patch.object(
            task.update_infra_deployments,
            "_collect_apply_result",
...
def test_run_update_infra_deployments_no_script(tmp_path: Path) -> None:
    ...
    params = _task_params(tmp_path, data_dir)
    with mock.patch(f"{TASK}.update_infra_deployments.git.clone") as clone:
...
def test_run_update_infra_deployments_bash_deprecation_warning(
    ...
    clone_dir = tmp_path / "cloned"
    with (
        mock.patch(f"{TASK}.update_infra_deployments.git.clone", return_value=clone_dir),
        mock.patch(f"{TASK}.update_infra_deployments.git.sync_to_origin_main"),
        mock.patch.object(task.update_infra_deployments, "_run_patched_script"),
        mock.patch.object(
            task.update_infra_deployments,
...
def test_run_update_infra_deployments_sandboxed_script(tmp_path: Path) -> None:
    ...
    with (
        mock.patch(f"{TASK}.update_infra_deployments.git.clone", return_value=clone_dir),
        mock.patch(f"{TASK}.update_infra_deployments.git.sync_to_origin_main"),
        mock.patch.object(task.update_infra_deployments, "execute_infra_updates") as execute,
        mock.patch.object(
            task.update_infra_deployments,
            "_collect_apply_result",
            return_value=task.ApplyResult(
...
def test_run_update_infra_deployments_sandboxed_preferred_over_bash(
    ...
    with (
        ...
        mock.patch(f"{TASK}.update_infra_deployments.git.sync_to_origin_main"),
        mock.patch.object(task.update_infra_deployments, "execute_infra_updates") as execute,
        mock.patch.object(task.update_infra_deployments, "_run_patched_script") as run_bash,
        mock.patch.object(
            task.update_infra_deployments,
            "_collect_apply_result",
            return_value=task.ApplyResult(
...
def test_run_update_infra_deployments_full_flow(tmp_path: Path) -> None:
    ...
    with (
        ...
        mock.patch.object(
            task.update_infra_deployments,
            "_snapshot_from_params",
            return_value=apply_result.snap,
        ),
        mock.patch.object(task.update_infra_deployments, "_run_patched_script"),
        mock.patch.object(
            task.update_infra_deployments, "_collect_apply_result", return_value=apply_result
        ),
        mock.patch.object(task.update_infra_deployments, "_create_or_update_pr") as create_pr,
        pytest.warns(DeprecationWarning, match="infra-deployment-update-script"),
    ):
        task.run_update_infra_deployments(params)
...
def test_params_from_env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    params = task.update_infra_deployments._params_from_env()
...
def test_main(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        task.update_infra_deployments, "run_update_infra_deployments", lambda _p: None
    )
    monkeypatch.setattr(
        task.update_infra_deployments, "_params_from_env", lambda: mock.MagicMock()
    )
    assert task.main() == 0
...
def test_main_entrypoint_exits_zero() -> None:
    ...
    with (
        mock.patch.object(
            task.update_infra_deployments, "_params_from_env", return_value=fake_params
        ),
        mock.patch.object(
            task.update_infra_deployments, "run_update_infra_deployments"
        ) as run,
    ):
...
def test_validate_path_rejects_absolute(tmp_path: Path) -> None:
    ...
    with pytest.raises(task.InfraReplacementError, match="absolute"):
        task.update_infra_deployments._validate_path("/etc/passwd", tmp_path)
...
def test_validate_path_rejects_traversal(tmp_path: Path) -> None:
    ...
    with pytest.raises(task.InfraReplacementError, match="traversal"):
        task.update_infra_deployments._validate_path("../escape", tmp_path)
...
def test_validate_path_accepts_relative(tmp_path: Path) -> None:
    ...
    target.parent.mkdir()
    target.touch()
    result = task.update_infra_deployments._validate_path("sub/file.yaml", tmp_path)
...
def test_replace_nth_match() -> None:
    ...
    content = "tag: a\ntag: b\ntag: c\n"
    result = task.update_infra_deployments._replace_nth_match(content, r"(tag: ).", r"\1X", 2)
    assert result == "tag: a\ntag: X\ntag: c\n"
...
def test_tag_oci_normalization() -> None:
    ...
    result = task.update_infra_deployments._tag("comp", r".*", snapshot_data=snapshot_data)
...
def test_tag_component_not_found() -> None:
    ...
    with pytest.raises(task.InfraReplacementError, match="not found"):
        task.update_infra_deployments._tag("missing", r".*", snapshot_data={"components": []})
...
def test_tag_no_image_tag() -> None:
    ...
    with pytest.raises(task.InfraReplacementError, match="no image tag"):
        task.update_infra_deployments._tag("comp", r".*", snapshot_data=snapshot_data)
...
def test_tag_filter_no_match() -> None:
    ...
    with pytest.raises(task.InfraReplacementError, match="does not match"):
        task.update_infra_deployments._tag("comp", r"\d+\.\d+", snapshot_data=snapshot_data)
...
def test_tag_invalid_filter_regex() -> None:
    ...
    with pytest.raises(task.InfraReplacementError, match="invalid filter regex"):
        task.update_infra_deployments._tag("comp", r"[bad", snapshot_data=snapshot_data)


def test_tag_components_not_a_list() -> None:
    """Treat non-list components as empty and raise not-found."""
    with pytest.raises(task.InfraReplacementError, match="not found"):
        task.update_infra_deployments._tag(
            "comp", r".*", snapshot_data={"components": "bogus"}
        )
...
def test_tag_skips_non_dict_entries() -> None:
    ...
    assert (
        task.update_infra_deployments._tag("comp", r".*", snapshot_data=snapshot_data) == "v1"
    )
...
def test_read_file(tmp_path: Path) -> None:
    ...
    target.write_text("hello\n", encoding="utf-8")
    result = task.update_infra_deployments._read("data.txt", clone_dir=tmp_path)
    assert result == "hello\n"
...
def test_read_file_not_found(tmp_path: Path) -> None:
    ...
    with pytest.raises(task.InfraReplacementError, match="file not found"):
        task.update_infra_deployments._read("missing.txt", clone_dir=tmp_path)


def test_read_path_traversal(tmp_path: Path) -> None:
    """Reject path traversal in read()."""
    with pytest.raises(task.InfraReplacementError, match="traversal"):
        task.update_infra_deployments._read("../escape", clone_dir=tmp_path)


def test_write_file(tmp_path: Path) -> None:
    """Write a file in the cloned repo."""
    task.update_infra_deployments._write("output.txt", "content\n", clone_dir=tmp_path)
...
def test_write_creates_parent_dirs(tmp_path: Path) -> None:
    ...
    task.update_infra_deployments._write("sub/dir/file.txt", "nested\n", clone_dir=tmp_path)
...
def test_write_path_traversal(tmp_path: Path) -> None:
    ...
    with pytest.raises(task.InfraReplacementError, match="traversal"):
        task.update_infra_deployments._write("../escape", "bad", clone_dir=tmp_path)
...
def test_replace_invalid_after_regex(tmp_path: Path) -> None:
    ...
    with pytest.raises(task.InfraReplacementError, match="invalid after regex"):
        task.update_infra_deployments._replace(
            "file.yaml", r"x", "y", clone_dir=tmp_path, revision="r", after=r"[bad"
        )
