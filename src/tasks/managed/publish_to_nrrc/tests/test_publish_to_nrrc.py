...
def _patch_file_type_cmd(
    ...
    *,
    ...
    monkeypatch.setattr(
        publish_to_nrrc.publish_to_nrrc.subprocess_cmd, "run_cmd", fake_run_cmd
    )
...
def test_prepare_repo_downloads_and_collects_archives(
    ...
    monkeypatch.setattr(
        publish_to_nrrc.publish_to_nrrc.oras_utils, "oras_pull", fake_oras_pull
    )
...
def test_main_success(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        publish_to_nrrc.publish_to_nrrc,
        "prepare_repo",
        lambda **_: None,
    )
...
def test_prepare_repo_oras_pull_failure_propagates(
    ...
    monkeypatch.setattr(
        publish_to_nrrc.publish_to_nrrc.oras_utils, "oras_pull", fail_oras_pull
    )
...
def test_main_oras_failure_exits(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(publish_to_nrrc.publish_to_nrrc, "prepare_repo", fail_prepare)
    with pytest.raises(subprocess.CalledProcessError):
        publish_to_nrrc.main()
...
def test_module_main_guard_raises_system_exit(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(SystemExit) as exc_info:
        runpy.run_path(
            str(Path(publish_to_nrrc.publish_to_nrrc.__file__)),
            run_name="__main__",
        )
    assert exc_info.value.code == 1
