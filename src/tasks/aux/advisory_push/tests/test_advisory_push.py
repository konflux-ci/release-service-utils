...
@patch("release_service_utils.helpers.http_client.get_retry_session")
@patch("release_service_utils.helpers.http_client.http_client.get_retry_session")
@patch(
...
def test_update_pyxis(
    ...
    mock_session_submodule: MagicMock,
    mock_session_package: MagicMock,
    tmp_path: Path,
) -> None:
...
"""
    ...
    # Mock HTTP session responses - both patches should return same session
    mock_session_obj = mock_session_package.return_value
    mock_session_submodule.return_value = mock_session_obj
    import json
    import requests
