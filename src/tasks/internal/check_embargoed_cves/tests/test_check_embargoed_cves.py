...
def test_fetch_flaw_state_empty_bodies() -> None:
    ...
    with mock.patch(
        "release_service_utils.helpers.osidb.fetch_flaw_response", return_value=""
    ) as m:
...
def test_main_passes_service_account_mount_resolved_by_environ(
    ...
    with mock.patch.object(
        check_embargoed_cves.check_embargoed_cves, "run_check", side_effect=_fake_run_check
    ):
...
def test_main_all_clear(
    ...
    with mock.patch.object(
        check_embargoed_cves.check_embargoed_cves,
        "run_check",
        return_value=([], 0),
    ) as run_mock:
...
def test_main_parse_args_non_int_exit_code_returns_one() -> None:
    ...
    with mock.patch.object(
        check_embargoed_cves.check_embargoed_cves, "parse_args", side_effect=SystemExit("x")
    ):
...
def test_main_returns_one_when_cve_list_is_empty_after_parsing() -> None:
    ...
    with mock.patch.object(
        check_embargoed_cves.check_embargoed_cves, "parse_args", return_value=ns
    ):
...
def test_main_missing_tekton_env() -> None:
    ...
    with mock.patch.object(
        check_embargoed_cves.check_embargoed_cves,
        "parse_args",
        return_value=mock.MagicMock(cves="CVE-1"),
    ):
        with pytest.raises(SystemExit) as e:
...
def test_module_main_guard_raises_system_exit(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(SystemExit) as e:
        runpy.run_path(
            str(Path(check_embargoed_cves.check_embargoed_cves.__file__)), run_name="__main__"
        )
    assert e.value.code == 1
