...
SOURCE_CONFIG_412 = {"config": {"Labels": {"com.redhat.index.delivery.version": "4.12"}}}
...
class TestWriteResult:
    ...
    def test_write_result_writes_to_file(self, tmp_path) -> None:
        """Test that write_result writes to result file."""
        result_path = tmp_path / "result.txt"
        write_result("Test message", result_path)
        assert result_path.read_text() == "Test message"
...
class TestMain:
    ...
    def _patch_main_deps(self, tmp_path) -> None:
        TASK = "release_service_utils.tasks.internal.publish_index_image.publish_index_image"
        self.result_path = tmp_path / "result.txt"
        with (
            ...
            mock.patch(f"{TASK}.tekton.result_paths_from_env") as self.result_paths,
        ):
            ...
            self.result_paths.return_value = (self.result_path,)
            yield

    def test_main_successful_copy(self) -> None:
        ...
        assert result == 0
        assert "Index Image Published successfully" in self.result_path.read_text()
        self.copy.assert_called_once()

    def test_main_digest_match_skip_copy(self) -> None:
        ...
        assert result == 0
        assert "already exists with the same digest" in self.result_path.read_text()
        self.copy.assert_not_called()

    def test_main_copy_failure(self) -> None:
        ...
        assert result == 0
        assert "Error: Failed publishing Index Image" in self.result_path.read_text()

    def test_main_credential_load_failure(self) -> None:
        ...
        assert result == 0
        assert "Credential file not found" in self.result_path.read_text()
    ...
    def test_main_ocp_version_mismatch(self) -> None:
        ...
        assert result == 0
        assert "does not match" in self.result_path.read_text()

    def test_main_target_not_found_copies(self) -> None:
