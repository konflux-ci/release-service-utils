...
from release_service_utils.helpers.skopeo import inspect, copy
...
def write_result(message: str, result_path: Path) -> None:
    """Write result message to Tekton result file.

    Args:
        message: The message to write.
        result_path: Path to the Tekton result file.
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--target-ocp-version",
        help="OCP Version this image was built to",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    (result_path,) = tekton.result_paths_from_env("RESULT_RESULT")
    ...
    try:
        ...
        source_ocp_version = (
            source_config["config"]["Labels"].get("com.redhat.index.delivery.version")
            if source_config
            else None
        )
        if source_ocp_version != args.target_ocp_version:
            message = (
                f"Error: The source index does not match its targetOcpVersion "
                f"({source_ocp_version} != {args.target_ocp_version})"
            )
            logger.error(message)
            write_result(message, result_path)
            return 0
        ...
        if target_image_info and re.match(r".*:v[0-9]{1}\.[0-9]{2}$", args.target_index):
            ...
            if source_ocp_version != real_target_ocp_version:
                message = (
                    f"Error: The indexes versions do not match "
        ...
        write_result(f"Error: {e}", result_path)
        return 0
