"""Verify every internal task subpackage is importable via its absolute path."""
...
def _discover_subpackages() -> list[str]:
    """Return internal task subpackage names that should be importable."""
    return sorted(
        d.name
...
def test_internal_task_importable(name: str) -> None:
    """Import ``release_service_utils.tasks.internal.<name>`` without error."""
    importlib.import_module(f"release_service_utils.tasks.internal.{name}")


@pytest.mark.parametrize("name", _discover_subpackages())
def test_internal_task_runnable_as_module(name: str) -> None:
    """Run ``python -m release_service_utils.tasks.internal.<name> --help``.

    without import errors.
    """
    module_name = f"release_service_utils.tasks.internal.{name}"
