...
from release_service_utils.helpers.tekton import tekton
...
from release_service_utils.tasks.internal import get_advisory_severity

TASK = "release_service_utils.tasks.internal.get_advisory_severity"
...
def _run_with_mocked_fetch(
    ...
    with (
        mock.patch(
            f"{TASK}.get_advisory_severity.authentication.kinit_with_retry",
            side_effect=_no_kinit,
        ),
        mock.patch(
            f"{TASK}.get_advisory_severity.fetch_flaws_parallel",
            return_value=flaw_cache,
        ),
    ):
        get_advisory_severity.get_advisory_severity.run_get_advisory_severity(
...
def test_unique_fixed_cves_skips_invalid_entries() -> None:
    ...
    assert get_advisory_severity.get_advisory_severity.unique_fixed_cves(images) == ["CVE-1"]


def test_higher_severity_unknown_levels_returns_current() -> None:
    """Unknown impact strings fall back to the current value."""
    assert (
        get_advisory_severity.get_advisory_severity.higher_severity("UNKNOWN", "ALSO")
        == "UNKNOWN"
    )
...
def test_purl_impact_entries_skips_invalid_rows() -> None:
    ...
    assert get_advisory_severity.get_advisory_severity.purl_impact_entries({}) == []
    flaw = {"affects": ["not-a-dict", {"purl": None}, {"purl": ""}]}
    assert get_advisory_severity.get_advisory_severity.purl_impact_entries(flaw) == []
...
def test_fetch_flaw_record_rejects_bad_response(body: str, match: str) -> None:
    ...
    with mock.patch(
        "release_service_utils.helpers.osidb.fetch_flaw_response", return_value=body
    ):
        with pytest.raises(ValueError, match=match):
            get_advisory_severity.get_advisory_severity.fetch_flaw_record(
                "https://osidb", "tok", "CVE-x"
            )
...
def test_fetch_flaws_parallel_empty_cve_list() -> None:
    ...
    assert (
        get_advisory_severity.get_advisory_severity.fetch_flaws_parallel(
            "https://osidb",
            [],
...
def test_fetch_flaws_parallel_populates_cache() -> None:
    ...
    cache = get_advisory_severity.get_advisory_severity.fetch_flaws_parallel(
        "https://osidb",
        ["CVE-a", "CVE-b"],
...
def test_process_cve_batch_skips_cached_cve() -> None:
    ...
    get_advisory_severity.get_advisory_severity._process_cve_batch(
        0,
        ["CVE-a", "CVE-b"],
        "https://osidb",
        cache,
...
def test_highest_severity_skips_invalid_image_rows() -> None:
    ...
    assert (
        get_advisory_severity.get_advisory_severity.highest_severity_for_images(images, {})
        == ""
    )
...
def test_module_main_guard_raises_system_exit(
    ...
    with mock.patch(f"{TASK}.get_advisory_severity.run_get_advisory_severity"):
        with pytest.raises(SystemExit) as exc:
            runpy.run_path(
                str(Path(get_advisory_severity.get_advisory_severity.__file__)),
                run_name="__main__",
            )
...
def test_decode_release_notes_images_roundtrip() -> None:
    ...
    assert (
        get_advisory_severity.get_advisory_severity.decode_release_notes_images(encoded)
        == images
    )
...
def test_decode_release_notes_images_rejects_non_array() -> None:
    ...
    with pytest.raises(ValueError, match="JSON array"):
        get_advisory_severity.get_advisory_severity.decode_release_notes_images(
            _gzip_b64({"x": 1})
        )
...
def test_unique_fixed_cves_stable_order() -> None:
    ...
    assert get_advisory_severity.get_advisory_severity.unique_fixed_cves(images) == [
        "CVE-b",
        "CVE-a",
        "CVE-c",
    ]
...
def test_higher_severity_picks_highest() -> None:
    ...
    assert (
        get_advisory_severity.get_advisory_severity.higher_severity("MODERATE", "CRITICAL")
        == "CRITICAL"
    )
    assert (
        get_advisory_severity.get_advisory_severity.higher_severity("CRITICAL", "LOW")
        == "CRITICAL"
    )
    assert (
        get_advisory_severity.get_advisory_severity.higher_severity("", "IMPORTANT")
        == "IMPORTANT"
    )
...
def test_resolve_impact_uses_general_when_component_empty() -> None:
    ...
    impact = get_advisory_severity.get_advisory_severity.resolve_impact_for_repository(
...
def test_highest_severity_catalog_happy_path() -> None:
    ...
    images = get_advisory_severity.get_advisory_severity.decode_release_notes_images(
        _gzip_b64(
            [
                {
                    "repository": "foo",
    ...
    assert (
        get_advisory_severity.get_advisory_severity.highest_severity_for_images(images, cache)
        == "CRITICAL"
    )
...
def test_fetch_flaw_record_parses_first_result() -> None:
    ...
    with mock.patch(
        ...
        row = get_advisory_severity.get_advisory_severity.fetch_flaw_record(
...
def test_fetch_flaw_with_token_retry_refreshes_token() -> None:
    ...
    record, token = get_advisory_severity.get_advisory_severity.fetch_flaw_with_token_retry(
        "https://osidb",
        "old-token",
        "CVE-1",
...
def test_fetch_flaw_with_token_retry_does_not_refresh_on_os_error() -> None:
    ...
    with pytest.raises(OSError, match="connection refused"):
        get_advisory_severity.get_advisory_severity.fetch_flaw_with_token_retry(
            "https://osidb",
            "old-token",
            "CVE-1",
            get_token=lambda _url: pytest.fail("get_token must not be called"),
...
def test_main_success(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _setup_main_env(tmp_path, monkeypatch)
    with mock.patch(f"{TASK}.get_advisory_severity.run_get_advisory_severity"):
        assert get_advisory_severity.get_advisory_severity.main() == 0
...
def test_main_writes_failure_result(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with mock.patch(
        f"{TASK}.get_advisory_severity.run_get_advisory_severity",
