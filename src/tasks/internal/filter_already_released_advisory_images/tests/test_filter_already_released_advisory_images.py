...
from release_service_utils.tasks.internal import (
    filter_already_released_advisory_images as filt,
)
...
def test_parse_args_ok() -> None:
    ...
    ns = filt.filter_already_released_advisory_images.parse_args(
        [
...
def test_parse_args_help_exits() -> None:
    ...
    with pytest.raises(SystemExit) as exc:
        filt.filter_already_released_advisory_images.parse_args(["--help"])
...
def test_parse_args_missing_required_exits() -> None:
    ...
    with pytest.raises(SystemExit) as exc:
        filt.filter_already_released_advisory_images.parse_args(["--origin", "only-origin"])
    assert exc.value.code == 1
...
def test_main_rejects_extra_argv(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    rc = filt.filter_already_released_advisory_images.main(
        [
            "prog",
            "--transformed-snapshot",
            "x",
...
def test_decode_transformed_snapshot_ok() -> None:
    ...
    assert (
        filt.filter_already_released_advisory_images.decode_transformed_snapshot(
...
def test_decode_transformed_snapshot_invalid_payload() -> None:
    ...
    with pytest.raises(ValueError, match="invalid transformed snapshot"):
        filt.filter_already_released_advisory_images.decode_transformed_snapshot("not-valid")
...
def test_encode_gzipped_base64_json_roundtrip() -> None:
    ...
    value = ["a", "b"]
    encoded = filt.filter_already_released_advisory_images.encode_gzipped_base64_json(value)
...
def test_unique_component_names_dedupes_and_skips_blank() -> None:
    ...
    assert filt.filter_already_released_advisory_images.unique_component_names(rows) == [
        "a",
        "b",
    ]
...
def test_filter_arch_images_triple_match() -> None:
    ...
    out = filt.filter_already_released_advisory_images.filter_arch_images(rows, existing)
...
def test_filter_arch_images_tag_mismatch_keeps_row() -> None:
    ...
    assert filt.filter_already_released_advisory_images.filter_arch_images(
        [row], existing
    ) == [row]
...
def test_list_advisory_subdirs_newest_first(tmp_path: Path) -> None:
    ...
    assert filt.filter_already_released_advisory_images.list_advisory_subdirs(base) == [
        "2025/200",
        "2024/100",
    ]
...
def test_list_advisory_subdirs_missing_dir() -> None:
    ...
    assert (
        filt.filter_already_released_advisory_images.list_advisory_subdirs(
            Path("/does/not/exist")
        )
        == []
    )
...
def test_advisory_errata_url_prefixes() -> None:
    ...
    assert (
        "access.redhat.com"
        in filt.filter_already_released_advisory_images.advisory_errata_url_prefix(
...
def test_build_advisory_urls_stage_and_prod() -> None:
    ...
    stage_public, stage_internal = (
        filt.filter_already_released_advisory_images.build_advisory_urls(
            "https://gitlab.com/rhtap-release/advisories.git",
    ...
    prod_public, _ = filt.filter_already_released_advisory_images.build_advisory_urls(
...
def test_run_filter_empty_advisory_directory(tmp_path: Path) -> None:
    ...
    out = filt.filter_already_released_advisory_images.run_filter(
...
def test_run_filter_uses_gitlab_clone_sparse(tmp_path: Path) -> None:
    ...
    with (
        mock.patch.object(
            filt.filter_already_released_advisory_images.gitlab, "configure_git_oauth2_auth"
        ),
        mock.patch.object(
            filt.filter_already_released_advisory_images.gitlab,
...
def test_main_writes_results(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    expected = filt.filter_already_released_advisory_images.FilterOutput(
        result="Success",
    ...
    with mock.patch.object(
        filt.filter_already_released_advisory_images, "run_filter", return_value=expected
    ):
        rc = filt.filter_already_released_advisory_images.main(
            [
...
def test_main_failure_still_exits_zero(
    ...
    paths = _result_env(monkeypatch, tmp_path)
    with mock.patch.object(
        filt.filter_already_released_advisory_images,
        "run_filter",
...
def test_main_missing_required_flags_returns_one() -> None:
    ...
    assert (
        filt.filter_already_released_advisory_images.main(
            ["filter_already_released_advisory_images.py", "--origin", "o"]
        )
        == 1
    )
...
def test_script_help_via_subprocess() -> None:
    ...
    script = Path(__file__).resolve().parents[1] / "filter_already_released_advisory_images.py"
    proc = subprocess.run(
        ...
        env={
            **os.environ,
            "PYTHONPATH": os.pathsep.join(
                [
                    str(Path(__file__).resolve().parents[4] / "helpers"),
