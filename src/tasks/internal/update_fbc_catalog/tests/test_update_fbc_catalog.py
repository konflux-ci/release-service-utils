...
from release_service_utils.tasks.internal import update_fbc_catalog

# ---------------------------------------------------------------------------
# parse_fbc_fragments
# ---------------------------------------------------------------------------

TASK = "release_service_utils.tasks.internal.update_fbc_catalog"


def test_parse_fbc_fragments_valid() -> None:
    """A valid JSON array is parsed and sorted."""
    assert update_fbc_catalog.update_fbc_catalog.parse_fbc_fragments('["b", "a", "c"]') == [
        "a",
        "b",
        "c",
    ]
...
def test_inspect_image_created_success() -> None:
    ...
    with mock.patch(
        ...
        assert (
            update_fbc_catalog.update_fbc_catalog.inspect_image_created("img:v1")
...
def test_inspect_image_created_skopeo_failure() -> None:
    ...
    with mock.patch(
        ...
        assert update_fbc_catalog.update_fbc_catalog.inspect_image_created("img:v1") is None
...
def test_is_build_newer_than_index_newer() -> None:
    ...
    with mock.patch(
        f"{TASK}.update_fbc_catalog.inspect_image_created",
        side_effect=[
            "2024-06-01T00:00:00Z",
            "2024-01-01T00:00:00Z",
        ],
    ):
        assert update_fbc_catalog.update_fbc_catalog.is_build_newer_than_index(
...
def test_is_build_newer_than_index_older() -> None:
    ...
    with mock.patch(
        ...
        assert not update_fbc_catalog.update_fbc_catalog.is_build_newer_than_index(
            build, "registry/idx:v4.12", "https://iib", "user"
        )
...
def test_is_build_newer_than_index_skopeo_fails_uses_iib() -> None:
    ...
    with (
        ...
        mock.patch(
            "release_service_utils.helpers.iib.query_builds",
            return_value={"items": []},
        ),
    ):
        assert not update_fbc_catalog.update_fbc_catalog.is_build_newer_than_index(
...
def test_is_build_newer_via_iib_no_newer_builds() -> None:
    ...
    with mock.patch(
        ...
        assert update_fbc_catalog.update_fbc_catalog._is_build_newer_via_iib(
            build, "registry/idx:v4.12", "https://iib", "user"
        )
...
def test_is_build_newer_via_iib_newer_build_exists() -> None:
    ...
    with mock.patch(
        ...
        assert not update_fbc_catalog.update_fbc_catalog._is_build_newer_via_iib(
            build, "registry/idx:v4.12", "https://iib", "user"
        )
...
def test_check_previous_build_finds_fresh_complete() -> None:
    ...
    with (
        ...
        mock.patch(
            f"{TASK}.update_fbc_catalog.is_build_newer_than_index",
            return_value=True,
        ),
    ):
        result = update_fbc_catalog.update_fbc_catalog.check_previous_build(
            "https://iib",
            "user",
            "idx:v4.12",
            ["a", "b"],
...
def test_poll_build_status_immediate_complete() -> None:
    ...
    build: iib.IIBBuild = {"id": 1, "state": "complete"}
    with mock.patch("release_service_utils.helpers.iib.get_build", return_value=build):
        result = update_fbc_catalog.update_fbc_catalog.poll_build_status(
            "https://iib",
            1,
            3600,
...
def test_poll_build_status_polls_until_complete() -> None:
    ...
    with mock.patch("release_service_utils.helpers.iib.get_build", side_effect=builds):
        result = update_fbc_catalog.update_fbc_catalog.poll_build_status(
            "https://iib",
            1,
            3600,
...
def test_poll_build_status_timeout() -> None:
    ...
    with mock.patch(
        "release_service_utils.helpers.iib.get_build",
        return_value={"id": 1, "state": "in_progress"},
    ):
        with pytest.raises(TimeoutError, match="Timeout after"):
            update_fbc_catalog.update_fbc_catalog.poll_build_status(
                "https://iib",
                1,
                60,
                sleep_fn=lambda _: None,
...
def test_poll_build_status_retries_on_fetch_error() -> None:
    ...
    with mock.patch("release_service_utils.helpers.iib.get_build", side_effect=_get_build):
        result = update_fbc_catalog.update_fbc_catalog.poll_build_status(
            "https://iib",
            1,
            3600,
...
def test_validate_index_image_overwrite_and_publish_match() -> None:
    ...
    update_fbc_catalog.update_fbc_catalog.validate_index_image(build, True, True)
...
def test_validate_index_image_overwrite_without_publish() -> None:
    ...
    with pytest.raises(ValueError, match="Invalid combination"):
        update_fbc_catalog.update_fbc_catalog.validate_index_image(build, True, False)


def test_validate_index_image_no_overwrite_passes() -> None:
    """No error for non-overwrite strategies."""
    build: iib.IIBBuild = {"id": 1, "state": "complete"}
    update_fbc_catalog.update_fbc_catalog.validate_index_image(build, False, True)
    update_fbc_catalog.update_fbc_catalog.validate_index_image(build, False, False)
...
def test_get_manifest_digests_success() -> None:
    ...
    with mock.patch(
        ...
        result = update_fbc_catalog.update_fbc_catalog.get_manifest_digests("img:v1")
...
def test_get_manifest_digests_not_multiarch() -> None:
    ...
    with mock.patch(
        ...
        with pytest.raises(RuntimeError, match="not multi-arch"):
            update_fbc_catalog.update_fbc_catalog.get_manifest_digests("img:v1")
...
def test_poll_and_collect_complete_success() -> None:
    ...
    with (
        mock.patch("release_service_utils.helpers.iib.get_build", return_value=full_build),
        mock.patch(
            "subprocess.run",
            return_value=_skopeo_result(json.dumps(manifest)),
        ),
    ):
        result = update_fbc_catalog.update_fbc_catalog._poll_and_collect(
            "https://iib",
            build,
            3600,
            True,
...
def test_poll_and_collect_timeout() -> None:
    ...
    with mock.patch(
        f"{TASK}.update_fbc_catalog.poll_build_status",
...
def test_poll_and_collect_missing_id() -> None:
    ...
    with pytest.raises(ValueError, match="missing 'id'"):
        update_fbc_catalog.update_fbc_catalog._poll_and_collect(
            "https://iib",
            build,
            3600,
            False,
...
def test_main_invalid_fbc_fragments(
    ...
    with pytest.raises(SystemExit, match="update_fbc_catalog.py"):
        update_fbc_catalog.update_fbc_catalog.main(
            [
                "--fbc-fragments",
                "not-json",
                "--from-index",
...
def test_main_writes_results_on_success(
    ...
    fake_result = update_fbc_catalog.update_fbc_catalog.RunResult(
    ...
    with mock.patch(
        f"{TASK}.update_fbc_catalog.run",
        return_value=fake_result,
    ):
        rc = update_fbc_catalog.update_fbc_catalog.main(
            [
                "--fbc-fragments",
                '["frag-a"]',
                "--from-index",
...
def test_main_writes_iib_log_when_url_empty(
    ...
    with mock.patch(f"{TASK}.update_fbc_catalog.run", return_value=fake_result):
        rc = update_fbc_catalog.update_fbc_catalog.main(
            ["--fbc-fragments", '["frag"]', "--from-index", "idx:v1"]
        )
...
def test_main_check_step_error_writes_failure(
    ...
    with mock.patch(
        "update_fbc_catalog.update_fbc_catalog.run",
...
def test_poll_and_collect_digest_extraction_fails() -> None:
    ...
    with (
        mock.patch("release_service_utils.helpers.iib.get_build", return_value=build),
        mock.patch(
            "subprocess.run",
            return_value=_skopeo_result(returncode=1),
        ),
...
def test_poll_and_collect_failed_state_reason() -> None:
    ...
    with mock.patch(
        "release_service_utils.helpers.iib.get_build", return_value=build_with_reason
    ):
        result = update_fbc_catalog.update_fbc_catalog._poll_and_collect(
            "https://iib",
            build_with_reason,
    ...
    with mock.patch(
        "release_service_utils.helpers.iib.get_build", return_value=build_no_reason
    ):
        result2 = update_fbc_catalog.update_fbc_catalog._poll_and_collect(
            "https://iib",
            build_no_reason,
            3600,
            False,
...
def test_is_build_newer_catalog_date_unparseable() -> None:
    ...
    with (
        ...
        mock.patch(
            f"{TASK}.update_fbc_catalog._is_build_newer_via_iib",
            return_value=True,
        ) as iib_mock,
    ):
        assert update_fbc_catalog.update_fbc_catalog.is_build_newer_than_index(
...
def test_poll_and_collect_fetches_full_details_for_reused_build() -> None:
    ...
    with (
        mock.patch(
            "release_service_utils.helpers.iib.get_build", return_value=detail_build
        ) as get_mock,
        mock.patch(
            "subprocess.run",
            return_value=_skopeo_result(json.dumps(manifest)),
