...
from release_service_utils.tasks.internal import process_file_updates
...
def test_parse_args_help() -> None:
    ...
    with pytest.raises(SystemExit) as exc:
        process_file_updates.process_file_updates.parse_args(["-h"])
...
def test_parse_args_missing_required() -> None:
    ...
    with pytest.raises(SystemExit) as exc:
        process_file_updates.process_file_updates.parse_args(
            ["--upstream-repo", "u", "--repo", "r"]
        )
    assert exc.value.code == 1
...
def test_parse_args_rejects_extra() -> None:
    ...
    with pytest.raises(SystemExit) as exc:
        process_file_updates.process_file_updates.parse_args([*_VALID_ARGS, "extra"])
...
def test_load_file_updates_secrets(secret_mount: Path) -> None:
    ...
    secrets = process_file_updates.process_file_updates.load_file_updates_secrets(secret_mount)
...
def test_load_file_updates_secrets_missing_file(tmp_path: Path) -> None:
    ...
    with pytest.raises(tekton.CheckStepError) as exc:
        process_file_updates.process_file_updates.load_file_updates_secrets(
            tmp_path / "missing"
        )
...
def test_gitlab_create_mr_returns_json() -> None:
    ...
    out = process_file_updates.process_file_updates.gitlab_create_mr(
        head="branch-1",
        title="t",
        target_branch="main",
        description="d",
...
def test_gitlab_create_mr_raises_when_web_url_missing() -> None:
    ...
    bare_mr.web_url = ""
    with pytest.raises(tekton.CheckStepError) as exc:
        process_file_updates.process_file_updates.gitlab_create_mr(
            head="b",
            title="t",
            target_branch="main",
            description="d",
...
def test_run_file_updates_short_circuit_on_path_outcome(
    ...
    with (
        mock.patch.object(
            process_file_updates.process_file_updates,
...
def test_git_functions_init_configures_identity() -> None:
    ...
    with mock.patch.object(
        process_file_updates.process_file_updates.vcs_git, "configure_git_global_user"
    ) as cfg:
...
def test_prepare_repository(tmp_path: Path) -> None:
    ...
    with (
        ...
        mock.patch.object(
            process_file_updates.process_file_updates.vcs_git, "rebase_onto_remote"
        ) as rebase,
    ):
...
def test_configure_git_environment(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with mock.patch.object(
        process_file_updates.process_file_updates.vcs_gitlab, "configure_git_oauth2_auth"
    ) as auth:
...
def test_write_json_error_result(tmp_path: Path) -> None:
    ...
    info = tmp_path / "info"
    state = tmp_path / "state"
    process_file_updates.process_file_updates.write_json_error_result(info, state, '"oops"')
    payload = json.loads(info.read_text(encoding="utf-8"))
    assert payload["error"] == '"oops"'
    assert state.read_text(encoding="utf-8") == "Failed"
...
def test_process_all_paths_returns_early_on_yaml_error(tmp_path: Path) -> None:
    ...
    with (
        mock.patch.object(process_file_updates.process_file_updates, "seed_target_file"),
        mock.patch.object(
            process_file_updates.process_file_updates,
...
def test_fetch_merge_request_head(tmp_path: Path) -> None:
    ...
    repo.mkdir()
    with mock.patch.object(
        process_file_updates.process_file_updates.vcs_git, "fetch"
    ) as fetch:
        ref = process_file_updates.process_file_updates._fetch_merge_request_head(repo, 42)
    assert ref == "mr_42"
...
def test_commit_and_create_mr(tmp_path: Path) -> None:
    ...
    with (
        mock.patch.object(
            process_file_updates.process_file_updates.vcs_git, "checkout"
        ) as checkout,
        mock.patch.object(
            process_file_updates.process_file_updates.vcs_git, "commit_staged"
        ) as commit,
        mock.patch.object(process_file_updates.process_file_updates.vcs_git, "push") as push,
        mock.patch.object(
            process_file_updates.process_file_updates,
            "gitlab_create_mr",
            return_value='{"merge_request":"https://x/mr/1"}',
        ),
        mock.patch.object(process_file_updates.process_file_updates.uuid, "uuid4") as uid,
    ):
...
def test_run_file_updates_no_cached_diff(tmp_path: Path) -> None:
    ...
    with (
        ...
        mock.patch.object(
            process_file_updates.process_file_updates, "get_cached_diff", return_value=""
        ),
    ):
        body, state, code = process_file_updates.run_file_updates(
            upstream_repo="u",
