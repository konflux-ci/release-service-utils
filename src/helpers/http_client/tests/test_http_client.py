...
def test_get_text_uses_session_and_headers() -> None:
    ...
    with mock.patch(
        "release_service_utils.helpers.http_client.http_client.get_retry_session",
...
def test_get_text_retries_429_then_succeeds() -> None:
    ...
    with (
        ...
        mock.patch(
            "release_service_utils.helpers.http_client.http_client.random.randint",
            return_value=0,
        ),
        mock.patch(
            "release_service_utils.helpers.http_client.http_client.time.sleep"
        ) as sleep_mock,
    ):
