...
def test_app_jwt_builds_token(tmp_path: Path) -> None:
    ...
    with mock.patch("release_service_utils.helpers.vcs.github.subprocess.run") as run:
...
def test_open_session(tmp_path: Path) -> None:
    ...
    with mock.patch.object(github, "app_jwt", return_value="jwt"):
        with mock.patch("release_service_utils.helpers.vcs.github.requests.post") as post:
...
def test_api_request_absolute_url_and_json_body() -> None:
    ...
    with mock.patch(
        "release_service_utils.helpers.vcs.github.requests.request", return_value=response
    ) as req:
...
def test_configure_git_askpass_auth(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with (
        mock.patch(
            "release_service_utils.helpers.vcs.github.tempfile.mkstemp",
            return_value=(fd, str(askpass_path)),
        ),
        mock.patch("release_service_utils.helpers.vcs.github.atexit.register") as register,
    ):
