...
from release_service_utils.helpers.extract_oci_artifacts import (
...
def test_process_component_raises_on_missing_container_image(
    ...
    """KeyError raised when component has files but no containerImage."""
    monkeypatch.setattr(extract_oci_artifacts, "CONTENT_DIR", tmp_path)
...
def test_process_component_happy_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_skopeo_copy(pullspec, dest_dir, *, extra_args=None, **kwargs):
    ...
    with mock.patch.object(extract_oci_artifacts.skopeo, "copy", side_effect=fake_skopeo_copy):
