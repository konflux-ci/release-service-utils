...
_DELETION_TIMEOUT_SECONDS = 60
_DELETION_POLL_INTERVAL_SECONDS = 2
...
def _wait_for_deletion(
    ...
    timeout: int = _DELETION_TIMEOUT_SECONDS,
) -> None:
    """Poll until an InternalRequest is fully removed (finalizers complete)."""
    ...
    logger.warning("Timed out waiting for InternalRequest %s to be fully deleted", ir_name)
