...
from release_service_utils.helpers.push_oci_unsigned import push_oci_unsigned
...
def test_run_skips_no_files_component(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with (
        ...
        mock.patch("release_service_utils.helpers.oras_utils.oras_push") as mock_push,
    ):
...
def test_run_moves_and_pushes_mac_and_windows(
    ...
    with (
        mock.patch("release_service_utils.helpers.oras_utils.oras_login"),
        mock.patch(
            "release_service_utils.helpers.oras_utils.oras_push", side_effect=fake_push
        ),
    ):
        push_oci_unsigned.run("quay.io/org", "uid-123")
