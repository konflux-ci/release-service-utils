...
from release_service_utils.helpers import push_artifacts
...
def _setup_secrets(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        push_artifacts.push_artifacts, "EXODUS_GW_SECRET_MOUNT", tmp_path / "exodus"
    )
    monkeypatch.setattr(push_artifacts.push_artifacts, "PULP_SECRET_MOUNT", tmp_path / "pulp")
    monkeypatch.setattr(
        push_artifacts.push_artifacts, "UDCACHE_SECRET_MOUNT", tmp_path / "udc"
    )
    monkeypatch.setattr(push_artifacts.push_artifacts, "CGW_SECRET_MOUNT", tmp_path / "cgw")
...
def test_check_cert_expiration_success() -> None:
    ...
    with mock.patch("subprocess.run") as mock_run:
        ...
        push_artifacts.push_artifacts._check_cert_expiration("/mnt/cert", 7)
...
def test_write_cert_files_creates_files(
    ...
    with mock.patch("pathlib.Path", wraps=Path):
        ec, ek, pc, pk, uc, uk = push_artifacts.push_artifacts._write_cert_files(
...
def test_create_exodus_conf_contains_required_fields(tmp_path: Path) -> None:
    ...
    push_artifacts.push_artifacts._create_exodus_conf(
        conf,
        Path("/tmp/exodus.crt"),
        Path("/tmp/exodus.key"),
...
def test_push_component_to_pulp_calls_pulp_push_wrapper(
    ...
    monkeypatch.setattr(push_artifacts.push_artifacts, "CONTENT_DIR", tmp_path / "artifacts")
    ...
    with mock.patch("pulp_push_wrapper.main") as mock_pulp:
        push_artifacts.push_artifacts._push_component_to_pulp(
            "testproduct",
            SNAPSHOT_STAGED,
...
def test_push_component_to_cdn_calls_rsync(
    ...
    with mock.patch("subprocess.check_call", side_effect=fake_check_call):
        push_artifacts.push_artifacts._push_component_to_cdn("prod", tmp_path / "exodus.conf")
...
def test_run_staged_calls_pulp(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(push_artifacts.push_artifacts, "CONTENT_DIR", tmp_path / "artifacts")
    monkeypatch.setattr(push_artifacts.push_artifacts, "SHARED_DIR", tmp_path / "shared")
...
def test_run_uses_shared_snapshot(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with mock.patch.object(
        push_artifacts.push_artifacts, "_check_cert_expiration", side_effect=fake_check_cert
    ):
        push_artifacts.run("pre", "https://cgw.example.com", 7)
...
def test_main_success() -> None:
    ...
    with mock.patch.object(push_artifacts.push_artifacts, "run") as mock_run:
