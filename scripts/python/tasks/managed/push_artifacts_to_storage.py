...
def run(
    *,
    ...
    if not snapshot_file.is_file():
        try:
            raise FileNotFoundError("No valid snapshot file was provided.")
        except FileNotFoundError as e:
            raise tekton.CheckStepError(str(e), e) from e
    if not data_file.is_file():
        try:
            raise FileNotFoundError("No data JSON was provided.")
        except FileNotFoundError as e:
            raise tekton.CheckStepError(str(e), e) from e
...
def main() -> int:
    ...
    data_dir = Path(os.environ.get("PARAM_DATA_DIR", DEFAULT_DATA_DIR))
