...
from decorators import async_in_executor
...
def main() -> int:
    ...
    for i, component in enumerate(components):
        ...
        target_ocp_version = component.get("ocp_version")

        log.debug(
            f"Extracted for component {i}: target_index={target_index}, "
            f"target_index_with_timestamp={target_index_with_timestamp}, "
            f"source_index={source_index}"
        )

        publishing_images = [target_index]
        publishing_sources = [source_index]
        ...
        if target_index != target_index_with_timestamp:
            publishing_images.append(target_index_with_timestamp)
            # Use resolved digest for timestamped tag to avoid race with parallel IIB builds
            publishing_sources.append(source_index_resolved)

        for pi, ps in zip(publishing_images, publishing_sources):
            params = [
                ("sourceIndex", ps),
                ("targetIndex", pi),
                ...
                ("targetOcpVersion", target_ocp_version or args.target_ocp_version),
            ]
            ...
            ir_futures[ft] = (target_index, ps, pi)
