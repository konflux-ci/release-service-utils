...
class BatchFailedError(Exception):
    """Raised when a batch execution fails and should be retried."""
...
def compute_target_index_with_timestamp(target_index: str, completion_time: str) -> str:
    ...
    (pattern: -<10_digit_epoch>). For staged releases, target_index is empty
    so result is also empty. Otherwise, append the completion_time.
    """
    if not target_index:
        return ""
    if len(target_index) >= 11 and target_index[-11] == "-" and target_index[-10:].isdigit():
        return target_index
...
def execute_batch_with_retry(
    ...
    """Execute a batch with retry using exponential backoff.

    Wraps execute_batch and converts failures to exceptions for retry handling.
    """

    def attempt() -> BatchResult:
        result = execute_batch(batch_num, from_index, group, config, data)
        if not result.success:
            raise BatchFailedError(result.error_message)
        return result
    ...
    return retry_helper.retry_with_exponential_backoff(
        ...
        retry_on=BatchFailedError,
        base_sleep_seconds=config.batch_retry_delay_seconds,
    )
...
def process_ocp_group(
    ...
    for batch_num in range(num_batches):
        ...
        try:
            batch_result = execute_batch_with_retry(
                batch_num,
                current_from_index,
                group,
                config,
            ...
            LOGGER.error(
                "Batch %d failed after all retries for OCP %s: %s",
                batch_num + 1,
                group.ocp_version,
                e,
            )
