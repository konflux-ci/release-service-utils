...
class BatchResult:
    ...
    success: bool
    index_image: str = ""
    build_info: IIBBuild = field(default_factory=dict)  # type: ignore[assignment]
    index_image_digests: list[str] = field(default_factory=list)
    iib_log: str = ""
    error_message: str = ""
...
def get_ocp_versions(snapshot: dict[str, Any]) -> list[str]:
    ...
    Components can have multiple versions as a JSON array: ["v4.17", "v4.18", "v4.19"].
    Falls back to empty list if ocpVersion is absent or null for backward compatibility.
    """
    ocp_versions: set[str] = set()
    for component in snapshot.get("components", []):
        ocp_version = component.get("ocpVersion", [])
        if isinstance(ocp_version, list):
            for version in ocp_version:
                if version:
                    ocp_versions.add(version)
        elif ocp_version:
...
def group_components_by_ocp_version(
    ...
    Expands multi-version components into separate instances per version using
    ocpVersionMetadata to get resolved indexes for each specific version.
    """
    ...
    for ocp_version in ocp_versions:
        ...
        for component in snapshot.get("components", []):
            ...
            if isinstance(comp_ocp_versions, str):
                comp_ocp_versions = [comp_ocp_versions] if comp_ocp_versions else []

            if ocp_version not in comp_ocp_versions:
                continue

            version_metadata = None
            for metadata in component.get("ocpVersionMetadata", []):
                if metadata.get("version") == ocp_version:
                    version_metadata = metadata
                    break

            expanded_component = dict(component)
            expanded_component["ocpVersion"] = ocp_version
            expanded_component["originalName"] = component.get("name", "")

            if version_metadata:
                expanded_component["updatedFromIndex"] = version_metadata.get(
                    "updatedFromIndex", ""
                )
                expanded_component["targetIndex"] = version_metadata.get("targetIndex", "")

            if len(comp_ocp_versions) > 1:
                expanded_component["name"] = f"{component.get('name', '')}-{ocp_version}"

            expanded_components.append(expanded_component)

        if not expanded_components:
            continue

        first_component = expanded_components[0]
        ...
        if target_index:
            if ":" not in target_index:
                raise ValueError(
                    f"targetIndex '{target_index}' for OCP version {ocp_version} "
                    "must include a tag (e.g., registry/repo:tag)"
                )
...
def calculate_timeouts(request_timeout_seconds: int) -> tuple[str, str, str]:
    """Calculate pipeline, task, and finally timeouts from request timeout.

    Returns (pipeline_timeout, task_timeout, finally_timeout) in XhYmZs format.
    """
    ...
    finally_minutes = finally_task_timeout // 60
    finally_seconds = finally_task_timeout % 60
    finally_timeout = f"0h{finally_minutes}m{finally_seconds}s"

    return pipeline_timeout, task_timeout, finally_timeout
...
def execute_batch(
    ...
    pipeline_timeout, task_timeout, finally_timeout = calculate_timeouts(
        request_timeout_seconds
    )

    params = {
        "fromIndex": from_index,
        "fbcFragments": json.dumps(fragments),
        "iibServiceAccountSecret": config.iib_service_account_secret,
        "publishingCredentials": publishing_credentials,
        "buildTimeoutSeconds": str(build_timeout_seconds),
        "buildTags": json.dumps(group.build_tags),
        "addArches": json.dumps(add_arches),
        "mustPublishIndexImage": str(config.must_publish_index_image).lower(),
        "mustOverwriteFromIndexImage": str(config.must_overwrite_from_index_image).lower(),
        "taskGitUrl": config.task_git_url,
        "taskGitRevision": config.task_git_revision,
    }

    labels = {
        TASK_LABEL: config.task_run_uid,
        PIPELINERUN_LABEL: config.pipeline_run_uid,
    }
    ...
    try:
        request_name = create(
            ...
            service_account=internal_request_service_account,
    ...
    exit_code_str = results.get("exitCode", "0")
    try:
    ...
    if exit_code != 0:
        ...
        return BatchResult(
            batch_num=batch_num,
            success=False,
            error_message=f"InternalRequest failed (exit code {exit_code}): {result_message}",
        )
    ...
    index_image = build_info.get("index_image") or ""
    ...
    return BatchResult(
        ...
        index_image_digests=index_image_digests,
        iib_log=iib_log,
    )


def compute_target_index_with_timestamp(target_index: str, completion_time: str) -> str:
    """Compute target_index_with_timestamp.

    For hotfix or pre-ga releases, target_index already has the timestamp
    (ends with 10+ digits). For staged releases, target_index is empty
    so result is also empty. Otherwise, append the completion_time.
    """
    if not target_index:
        return ""
    if len(target_index) >= 10 and target_index[-10:].isdigit():
        return target_index
    return f"{target_index}-{completion_time}"
...
def process_batch_results(
    ...
    Returns True if results were successfully processed, False otherwise.
    A False return indicates the batch should be treated as failed.
    """
    if not batch_result.success or not batch_result.build_info:
        return False

    build_info = batch_result.build_info
    ...
    target_index_with_timestamp = compute_target_index_with_timestamp(
        group.target_index, completion_time
    )
    ...
    for fragment in fragments:
        component_result = {
            ...
            "image_digests": batch_result.index_image_digests,
            "index_image": build_info.get("index_image", ""),
            "index_image_resolved": build_info.get("internal_index_image_copy_resolved") or "",
            "completion_time": completion_time,
            "iibLog": batch_result.iib_log,
        }
...
def process_ocp_group(
    ...
    Executes all batches first, collects failures, then retries failed batches
    at the end (giving IIB time to recover from transient failures).
    ...
    for batch_num in range(num_batches):
        ...
        if batch_result.success:
            results_ok = process_batch_results(
                batch_result,
                group,
                config,
                results_data,
...
def run(config: AddFBCContributionConfig) -> tuple[dict[str, Any], str]:
