...
from collect_slack_notification_params import ReleaseMetadata, build_slack_message

import send_slack_notification

SCRIPT_PATH = Path(__file__).with_name("send_slack_notification.py")
...
def test_post_slack_webhook_posts_json_body() -> None:
    ...
    with mock.patch(
        "send_slack_notification.requests.post",
...
def test_run_skips_when_message_empty(tmp_path: Path) -> None:
    ...
    with mock.patch("send_slack_notification.post_slack_webhook") as post:
