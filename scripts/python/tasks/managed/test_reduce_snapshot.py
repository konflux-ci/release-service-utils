...
def test_resolve_namespace_provided() -> None:
    ...
    assert reduce_snapshot.resolve_namespace("my-ns") == "my-ns"
...
def test_resolve_namespace_from_service_account(tmp_path: Path) -> None:
    ...
    with mock.patch.object(reduce_snapshot, "_SA_NAMESPACE_PATH", ns_file):
        assert reduce_snapshot.resolve_namespace("") == "sa-namespace"


def test_resolve_namespace_missing_file() -> None:
    """Raise ValueError when namespace is empty and file is unreadable."""
    with mock.patch.object(reduce_snapshot, "_SA_NAMESPACE_PATH", Path("/nonexistent/path")):
        with pytest.raises(ValueError, match="cannot read"):
            reduce_snapshot.resolve_namespace("")
...
def test_get_cr_labels_parses_json() -> None:
    ...
    with mock.patch("reduce_snapshot.get_resource", return_value=raw):
        labels = reduce_snapshot.get_cr_labels("snapshot", "ns", "name")
    assert labels == {
        "test.appstudio.openshift.io/type": "component",
        "appstudio.openshift.io/component": "foo",
    }
...
def test_get_cr_labels_empty_response() -> None:
    ...
    with mock.patch("reduce_snapshot.get_resource", return_value="{}"):
        assert reduce_snapshot.get_cr_labels("snapshot", "ns", "name") == {}
...
def test_validate_labels_valid() -> None:
    ...
    assert reduce_snapshot.validate_labels(labels) == "my-comp"
...
def test_validate_labels_wrong_type() -> None:
    ...
    with pytest.raises(ValueError, match="missing the required labels"):
        reduce_snapshot.validate_labels(labels)
...
def test_validate_labels_empty() -> None:
    ...
    with pytest.raises(ValueError, match="missing the required labels"):
        reduce_snapshot.validate_labels({})
...
def test_reduce_snapshot_success() -> None:
    ...
    result = reduce_snapshot.reduce_snapshot(snapshot, "tom")
...
def test_reduce_snapshot_component_not_found() -> None:
    ...
    result = reduce_snapshot.reduce_snapshot(snapshot, "scoobydoo")
...
def test_reduce_snapshot_no_components_key() -> None:
    ...
    assert reduce_snapshot.reduce_snapshot(snapshot, "foo") is snapshot
...
def test_run_reduces_snapshot(tmp_path: Path) -> None:
    ...
    with mock.patch("reduce_snapshot.get_resource", return_value=labels_json):
        reduce_snapshot.run(
...
def test_run_empty_namespace_reads_sa(tmp_path: Path) -> None:
    ...
    with (
        mock.patch.object(reduce_snapshot, "_SA_NAMESPACE_PATH", ns_file),
        mock.patch("reduce_snapshot.get_resource", return_value=labels_json) as mock_gr,
    ):
        reduce_snapshot.run(
            single_component="true",
            custom_resource="snapshot/snap1",
