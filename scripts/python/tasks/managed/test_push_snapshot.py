...
class TestSelectOciAuth:
    ...
    def test_returns_stdout(self) -> None:
        ...
        with patch("push_snapshot.subprocess_cmd.run_cmd") as mock_run:
...
class TestCreateSourceAuthFile:
    ...
    def test_remaps_registry_to_repo(self, tmp_path: Path) -> None:
        ...
        with patch("push_snapshot.select_oci_auth", return_value=auth_json):
            with patch("push_snapshot.file.make_tempfile_path") as mock_tmp:
    ...
    def test_handles_empty_auth(self, tmp_path: Path) -> None:
        ...
        with patch("push_snapshot.select_oci_auth", return_value="{}"):
            with patch("push_snapshot.file.make_tempfile_path") as mock_tmp:
                tmp_file = tmp_path / "auth.json"
                mock_tmp.return_value = tmp_file
...
class TestDiscoverArtifactsWithRetry:
    ...
    def test_returns_count_on_success(self, tmp_path: Path) -> None:
        ...
        auth_file.write_text("{}", encoding="utf-8")
        with patch(
            "push_snapshot.oras_discover_referrers",
...
class TestPushImage:
    ...
    def test_performs_cosign_copy(self, tmp_path: Path) -> None:
        ...
        with patch("push_snapshot.create_dest_auth_file") as mock_dest_auth:
            ...
            with patch("push_snapshot.oras_utils.oras_resolve", return_value=None):
                with patch("push_snapshot.create_combined_docker_config") as mock_config:
                    config_dir = tmp_path / "docker"
                    config_dir.mkdir()
                    (config_dir / "config.json").write_text("{}")
                    mock_config.return_value = config_dir

                    with patch("push_snapshot.cosign_copy") as mock_cosign:
                        result = push_snapshot.push_image(
                            push_snapshot.PushJob(
                                origin_digest="sha256:abc123",
                                name="comp1",
        ...
        with patch("push_snapshot.create_dest_auth_file") as mock_dest_auth:
            ...
            with patch("push_snapshot.oras_utils.oras_resolve", return_value=None):
                with patch("push_snapshot.create_combined_docker_config") as mock_config:
                    ...
                    with patch(
                        "push_snapshot._discover_artifacts_with_retry",
                        return_value=2,
                    ):
                        with patch("push_snapshot.oras_utils.oras_cp") as mock_oras:
                            result = push_snapshot.push_image(
                                push_snapshot.PushJob(
    ...
    def test_skips_when_already_exists(self, tmp_path: Path) -> None:
        ...
        with patch("push_snapshot.create_dest_auth_file") as mock_dest_auth:
            ...
            with patch(
                ...
                with patch("push_snapshot.oras_utils.oras_cp") as mock_cp:
...
def run_mocks(tmp_path: Path) -> Generator[types.SimpleNamespace, None, None]:
    ...
    with (
        patch(
            "push_snapshot.create_source_auth_file",
            return_value=src_auth,
        ) as m_auth,
        patch("push_snapshot.get_image_architectures") as m_arch,
        patch("push_snapshot.skopeo.inspect") as m_inspect,
        patch("push_snapshot.oras_utils.oras_resolve") as m_resolve,
        patch("push_snapshot.push_image") as m_push,
        patch("push_snapshot.push_migration_artifact") as m_mig,
    ):
...
class TestRun:
    ...
    def test_burst_triggers_stabilization_delay(self, tmp_path: Path, run_mocks) -> None:
        ...
        with patch("push_snapshot.time.sleep") as mock_sleep:
            push_snapshot.run(snapshot_file, data_file, results_dir, 5, 0, False)
...
class TestMain:
    ...
    def test_success(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        ...
        with patch("push_snapshot.authentication.setup_ca_cert"):
            with patch("push_snapshot.run") as mock_run:
                push_snapshot.main()
