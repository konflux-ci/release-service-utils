...
class TestInspectImage:
    ...
    @mock.patch("publish_index_image.inspect")
...
class TestCopyImage:
    ...
    @mock.patch("publish_index_image.copy")
...
class TestMain:
    ...
    def _patch_main_deps(self) -> None:
        with (
            mock.patch("publish_index_image.parse_arguments") as self.parse_args,
            mock.patch(
                "publish_index_image.setup_logger",
                return_value=mock.Mock(),
            ) as self.setup_logger,
            mock.patch("publish_index_image.load_credential") as self.load_cred,
            mock.patch("publish_index_image.inspect") as self.inspect,
            mock.patch("publish_index_image.copy") as self.copy,
        ):
    ...
    def test_main_ocp_version_mismatch(self, capsys) -> None:
        """Test main exits when source OCP version mismatches target."""
        self.parse_args.return_value = self._mock_args(
