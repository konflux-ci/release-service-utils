...
def inspect_target_image(
    ...
    """
    logger.info(f"Getting target image digest: {target_index}")
    try:
        digest = client.inspect(
            f"docker://{target_index}",
            format="{{.Digest}}",
            creds=target_credential,
...
def main() -> int:
    ...
    try:
        ...
        # Step 5: Check target image existence
        target_digest = inspect_target_image(
            client,
            args.target_index,
            target_credential,
            logger=logger,
        ...
        # Step 6 & 7: Compare digests if target exists
