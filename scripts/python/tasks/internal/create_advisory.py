...
* Reads `/mnt/advisory_secret` (`gitlab_host`, `gitlab_access_token`,
  `git_author_name`, `git_author_email`, `git_repo`) and
  `/mnt/errata_secret` (`errata_api`, Kerberos keytab).
...
def _reserve_errata_live_id(
    ...
    *,
    ...
    try:
        ...
        # urllib3 retries cover transient connection and HTTP errors on POST.
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        run_create_advisory(
            advisory_secret=Path("/mnt/advisory_secret"),
            errata_mount=Path("/mnt/errata_secret"),
