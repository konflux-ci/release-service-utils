...
class FakeSkopeoClient:
    ...
    def inspect(
        ...
        *,
        ...
        if raw:
            actual_params["raw"] = raw
        if config:
            actual_params["config"] = config
        ...
        res = rule["return"]
        if format is None and isinstance(res, dict):
            if raw:
                return ContainerImageRaw.from_dict(res)
            if config:
                return ContainerImageConfig.from_dict(res)
            return ContainerImage.from_dict(res)
        return res
