...
def _os_target_dir(
    os_name: str, arch: str, component_dir: Path, unsigned_dir: Path
) -> Path | None:
    """Return the target directory for a given OS/arch combination, or None if unknown."""
    if os_name == "darwin":
        return unsigned_dir / "macos" / arch
    elif os_name == "linux":
        return component_dir / "linux" / arch
    elif os_name == "windows":
        return unsigned_dir / "windows" / arch
    return None


def _process_file_entries(
    entries: list[dict], component_dir: Path, unsigned_dir: Path
) -> None:
    ...
    for entry in entries:
        ...
        target_dir = _os_target_dir(os_name, arch, component_dir, unsigned_dir)
...
def _push_oras(tag: str, directory: Path, subdirectory: str, component_name: str) -> str:
    """Push a subdirectory to Quay via oras and return the digest."""
...
def run(quay_url: str, pipeline_run_uid: str) -> None:
    ...
    for component in snapshot.get("components", []):
        ...
        _process_file_entries(component.get("files") or [], component_dir, unsigned_dir)
        ...
        if has_mac:
            ...
            mac_digest = _push_oras(tag, unsigned_dir, "macos", name)
        ...
        if has_windows:
            ...
            win_digest = _push_oras(tag, unsigned_dir, "windows", name)
