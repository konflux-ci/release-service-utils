...
def test_is_disk_image_file_unambiguous_extensions() -> None:
    """is_disk_image_file returns True for all supported unambiguous disk-image suffixes."""
    assert disk_image_utils.is_disk_image_file("disk.qcow2")
    assert disk_image_utils.is_disk_image_file("install.iso")
    assert disk_image_utils.is_disk_image_file("install.iso.gz")
    assert disk_image_utils.is_disk_image_file("disk.raw.gz")
    assert disk_image_utils.is_disk_image_file("disk.vhd.gz")
    assert disk_image_utils.is_disk_image_file("DISK.QCOW2")


def test_is_disk_image_file_non_disk_image() -> None:
    """is_disk_image_file returns False for binary archives and ambiguous extensions."""
    assert not disk_image_utils.is_disk_image_file("binary-linux-amd64.tar.gz")
    assert not disk_image_utils.is_disk_image_file("image.tar.gz")
    assert not disk_image_utils.is_disk_image_file("binary.zip")
    assert not disk_image_utils.is_disk_image_file("binary.tar")
