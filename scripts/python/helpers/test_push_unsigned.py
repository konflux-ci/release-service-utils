...
def test_process_file_entries_linux(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    push_unsigned._process_file_entries(
...
def test_process_file_entries_darwin(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(push_unsigned, "CONTENT_DIR", tmp_path)
    comp_dir = tmp_path / "prod"
    comp_dir.mkdir()
...
def test_process_file_entries_windows(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(push_unsigned, "CONTENT_DIR", tmp_path)
    comp_dir = tmp_path / "prod"
    comp_dir.mkdir()
    unsigned_dir = comp_dir / "unsigned"
...
def test_process_file_entries_missing_archive_is_warned(
...
def test_process_file_entries_skips_missing_fields(tmp_path: Path) -> None:
    push_unsigned._process_file_entries(
        [{"os": "linux"}],
        tmp_path,
        tmp_path / "unsigned",
...
def test_process_file_entries_rejects_path_traversal(tmp_path: Path) -> None:
    comp_dir = tmp_path / "prod"
    comp_dir.mkdir()
    archive = comp_dir / "binary-linux-amd64.tar.gz"
...
def test_push_oras_returns_digest(tmp_path: Path) -> None:
    with mock.patch("subprocess.check_output", return_value="Digest: sha256:abc123\n"):
        digest = push_unsigned._push_oras("quay.io/org/prod:tag", tmp_path, "macos", "prod")
    assert digest == "sha256:abc123"


def test_push_oras_raises_on_missing_digest(tmp_path: Path) -> None:
    with mock.patch("subprocess.check_output", return_value="no digest here"):
        with pytest.raises(RuntimeError, match="digest"):
            push_unsigned._push_oras("quay.io/org/prod:tag", tmp_path, "macos", "prod")
