...
class SkopeoClientError(Exception):
    ...
    def __str__(self) -> str:
        ...
        return (
            f"{super().__str__()}\n"
            f"    Command: {' '.join([str(x) for x in self.command])}\n"
...
class SkopeoClient:
    ...
    Example:
        ...
        from rsutils.secret import Secret
        client.copy(
    ...
    def inspect(
        ...
        *,
        ...
        Returns:
            dict if format is None, str if format is specified
        ...
        try:
            ...
            raise SkopeoClientError(
                f"Failed to parse skopeo output as JSON: {e}",
                command=cmd,
                returncode=0,
                stdout=output,
