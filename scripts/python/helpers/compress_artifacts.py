...
def _arch_dir(os_name: str, arch: str, component_dir: Path) -> Path | None:
    """Return the signed-content directory for a given OS/arch combination, or None."""
    signed_dir = component_dir / "signed"
    if os_name == "darwin":
        return signed_dir / "macos" / arch
    elif os_name == "linux":
        return component_dir / "linux" / arch
    elif os_name == "windows":
        return signed_dir / "windows" / arch
    return None
...
def _process_file_entry(
    ...
    arch_dir = _arch_dir(os_name, arch, component_dir)
    if arch_dir is None:
...
def compress_component(component: dict, snapshot: dict) -> dict:
    ...
    if files_entries:
        ...
        for entry in files_entries:
            normalized_source = _process_file_entry(entry, "files", component_dir, ready_dir)
    ...
    if staged_entries:
        ...
        for entry in staged_entries:
            _process_file_entry(entry, "staged.files", component_dir, ready_dir)
