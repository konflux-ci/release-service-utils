...
def _kinit(checksum_user: str, kerberos_realm: str, keytab_b64: bytes) -> None:
    ...
    keytab = Path(tempfile.mktemp(suffix=".keytab"))
    try:
        keytab.write_bytes(base64.b64decode(keytab_b64))
        subprocess.check_call(
            ["kinit", "-kt", str(keytab), f"{checksum_user}@{kerberos_realm}"]
        )
    finally:
